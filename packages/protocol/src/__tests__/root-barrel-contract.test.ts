...
describe('protocol root barrel contract', () => {
  ...
  it('exports minimal WebSocket request contracts from the root barrel', () => {
    ...
    expect(getWsRequestContract('session_goal_control')).toMatchObject({
      commandType: 'session_goal_control',
      resultFamily: 'session_goal_control',
      successEvents: ['session_goal_snapshot'],
      errorCodeFragments: ['session_goal_control'],
    })
  ...
  it('pins request-id-bearing ClientCommand variants and proves requestId payloads are accepted', () => {
    ...
    expect(REQUEST_ID_COMMAND_TYPES).toHaveLength(58)
