...
| **Fast Mode / service tier** | OpenAI Codex session/runtime path | Chat header, settings auth/model refresh, specialist runtime propagation | Fast Mode is session-scoped, inherited by compatible OpenAI Codex workers/specialists, and safely ignored for incompatible providers or models. Explicit specialist opt-out must survive fallback recovery, and OAuth or credential-pool mutations should refresh eligibility in the UI. |
