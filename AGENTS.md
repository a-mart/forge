...
| **Cursor SDK runtime** | `swarm/runtime/cursor-sdk/` | Settings UI (manager and specialist selectors) | Native Cursor SDK agent runtime via `@cursor/sdk` for manager and specialist agents. Uses API-key auth from `CURSOR_API_KEY` in Settings → Authentication, secrets, or env, persists Forge-owned `stateRoot` and `sdkAgentId`. Electron packaging stages and preflights `@cursor/sdk`, `sqlite3`, and platform binaries. |
