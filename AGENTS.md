...
| **Working plans** | `swarm/planning/` | `components/chat/plan/`, chat header | Builder managers have one always-available `update_plan` tool with the Codex-style full-snapshot schema: optional explanation plus ordered steps in `pending`, `in_progress`, or `completed` state, with at most one in progress. The current plan persists in per-session `plan.json`; the latest revision is injected as bounded runtime-only context on manager turns and preserved through compaction so runtime replacement does not lose it. There are no plan IDs, task IDs, modes, worker links, history receipts, settings toggle, or Collaboration surface. Clearing a conversation clears its plan, while stop/archive preserves it and forks start without copying it. |
