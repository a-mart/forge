...
| **Active Work Plans (parked)** | `swarm/coordination/`, `swarm/skills/builtins/active-work-plans/SKILL.md` | `components/chat/active-work/`, chat header, `components/chat/message-list/WorkPlanCreatedRow.tsx` | The implementation and compatibility data remain, but the current runtime hard-disables this feature in `coordination/work-plans-settings.ts`: reads return disabled, guidance is empty, and setting writes are ignored while any existing settings file is preserved. Managers do not receive the `task` tool/guidance and the live Active Work UI is unavailable. Existing session `tasks.json` sidecars and historical `work_plan_created` receipts remain preserved for compatibility. |
