...
| **File browser / inline editor** | `ws/http/routes/file-browser-routes.ts`, `ws/http/services/file-browser-service.ts` | `components/file-browser/*`, `hooks/index-page/use-panel-state.ts` | Preview-first repository browser with desktop-only single-file inline editing. Edit mode lazy-loads the CodeMirror editor. `/api/files/content` supports versioned `GET`/`PUT` saves with required `baseVersion`, strict stale-version conflicts, and explicit overwrite handling; do not allow silent overwrite paths. Dirty guards cover file switches, panel close, session switches, and Source Control transitions. Preserve exact path whitespace for file names with leading/trailing spaces when reading or saving. |
