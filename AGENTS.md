...
| **Cortex knowledge (v2)** | `swarm/knowledge-service.ts`, `swarm/knowledge-consolidator-api.ts`, `swarm/capture-check.ts`, `swarm/swarm-cortex-service.ts`, `swarm/cortex-auto-review-settings.ts`, `swarm/knowledge-v2-{migration-manifest,migration-lock,settings-service}.ts`, `ws/http/routes/knowledge-v2-settings-routes.ts`, `ws/http/routes/cortex-auto-review-routes.ts` | `components/chat/cortex/` (Index/Entries/Changelog/Consolidation tabs), `components/settings/{cortex-auto-review-api,knowledge-v2-api}.ts` | Default-off preview with provenance-bearing global/profile entries (`shared/knowledge/entries/`, `profiles/<id>/knowledge/entries/`) and token-capped indexes (global 1,500 / profile 800 tok). When ON, prompts inject global/profile `INDEX.md` plus session `memory.md`; canonical profile `memory.md` and legacy `shared/knowledge/common.md` remain maintained but are not injected. OFF restores legacy common + profile + session injection without deleting v2 files. `knowledge` (search/read) and manager-only `save_learning` operate on entries; capture checks propose missed durable facts, while the consolidator only reads entries to merge/archive/reindex and never mines transcripts or creates entries. False→true activation requires a strictly valid completed v1/v2 migration manifest while holding the ownership-safe cross-process migration/activation lock. New migrations atomically write a truthful v2 `authorized_pending` manifest before guarded activation; failure stays OFF. Capability reads fail closed, unsafe PUT returns 409 `KNOWLEDGE_V2_MIGRATION_REQUIRED`, and Settings/onboarding do not start migration or issue unsafe activation. Replaces the deleted transcript-review fleet. |
...
│   │   ├── common.md                      #   Legacy common knowledge; maintained in both modes, prompt-injected only with v2 OFF
