...
| **Memory system** | `swarm/memory-merge.ts`, `swarm/storage/data-paths.ts` (path resolution) | Chat UI | Per-session and per-profile persistent memory with merge lifecycle. Orthogonal to Cortex knowledge (v2): session/profile `memory.md` is unaffected by the `knowledgeV2.enabled` switch. |
| **Cortex knowledge (v2)** | `swarm/knowledge-service.ts`, `swarm/knowledge-consolidator-api.ts`, `swarm/capture-check.ts`, `swarm/swarm-cortex-service.ts`, `swarm/cortex-auto-review-settings.ts`, `ws/http/routes/knowledge-v2-settings-routes.ts`, `ws/http/routes/cortex-auto-review-routes.ts` | `components/chat/cortex/` (Index/Entries/Changelog/Consolidation tabs), `components/settings/cortex-auto-review-api.ts` | Per-entry knowledge files with provenance (`shared/knowledge/entries/`, `profiles/<id>/knowledge/entries/`), each capped and rolled into a token-capped `INDEX.md` (global 1,500 / profile 800 tok) that is the only surface injected into prompts. `knowledge` (search/read) and `save_learning` (manager-only) tools write entries directly; a consolidator agent (`archetypes/builtins/cortex.md`) merges/archives/reindexes on a cadence — it only reads entries, never transcripts, and cannot create entries. A capture cascade (deterministic cadence check → cheap-model judge → restricted-tool fork) proposes new entries at compaction/idle/close. Gated behind `knowledgeV2.enabled` (default **off**); with the switch off, prompt assembly reverts unchanged to legacy `common.md`/`memory.md`. Replaces the earlier transcript-review fleet (`cortex-scan`, review-runs, `ReviewStatusPanel`), which is deleted. |
...
| **Message routing** | `swarm/message-router.ts`, `swarm/turn-ledger.ts` (`worker_report` as a `conversation_message` source) | All view / worker drill-down only | `MessageRouter.resolve()` is the single choke point for render/route/drop decisions — visible-by-default for user-facing origin, each decision reason-coded (`render:*`/`route:*`/`deny:*`) and receipted. Worker completions are always emitted as `worker_report` (persisted, receipted, excluded from model context) but **never render in the default manager view**, only All view/worker drill-down. A manager turn that ends with no visible entry triggers a runtime-agnostic backstop notice, enriched with a deep link if a worker report went unsummarized. Replaces the deleted `inheritedAssistantOutputTargetByWorkerId` target-inheritance plumbing. |
| **Liveness / turn watchdog** | `swarm/manager-turn-watchdog.ts`, `swarm/turn-ledger.ts`, `ws/http/routes/restart-recovery-routes.ts` | Top-of-app restart-recovery banner | Manager-turn deadline watchdog (arms on turn accept, tool execution pauses the clock) escalates 30s → 5m → 10m: notice → detailed notice → offer to recycle the runtime. A per-session `turns.jsonl` ledger records dispatch/delivery/terminal events; boot reconciliation replays it after a crash and offers one-click resume. Suppression flags are TTL'd so a wedged flag self-clears. Orthogonal to the worker-side stall/idle detectors above, which are unchanged. |
...
| **File browser / inline editor** | `ws/http/routes/file-browser-routes.ts`, `ws/http/services/file-browser-service.ts` | `components/file-browser/*`, `hooks/index-page/use-panel-state.ts` | Repository browser with desktop-only direct single-file text editing. Supported editable text files open directly in CodeMirror with syntax highlighting; editable Markdown defaults to source/editor mode. Non-editable files and mobile stay read-only. `/api/files/content` supports versioned `GET`/`PUT` saves with required `baseVersion`, strict stale-version conflicts, explicit overwrite handling, and `DELETE` for files/folders; do not allow silent overwrite paths or workspace-root/traversal deletion. Deletes are scoped to the active cwd/worktree, preserve exact path whitespace, unlink symlinks instead of following targets, normalize Windows path separators for symlink-walk safety, and refresh file tree/content caches plus Source Control after success. Dirty guards cover file switches, panel close, session switches, Source Control transitions, and deletes of the open file or ancestor folders. |
| **Source Control workspace** | `ws/http/routes/git-source-control-routes.ts`, `ws/http/services/git-source-control-service.ts`, `ws/http/services/git-hosted-provider.ts`, `versioning/git-cli.ts` | `components/diff-viewer/*`, `components/file-browser/*`, `hooks/index-page/use-panel-state.ts` | Desktop workspace rail Source Control surface for selected-worktree Changes, History, Worktrees, and Pull Requests. Selecting a worktree scopes Source Control and Files browsing/editing without changing chat session CWD. Entering Source Control or changing context may guarded-auto-fetch stale origin data; auto-fetch failures stay quiet/non-blocking, while manual fetch errors surface to the user. Branch actions support fetch, switch, create, and fast-forward-only pull with confirmation plus head/status preflight. Same-workspace file saves and deletes refresh Source Control; same-worktree dirty inline edits guard Source Control transitions, mutations, and affected file/folder deletes. Pull Requests use GitHub `gh`, lazy-load open-count badge data after the PR tab is visited, and degrade when no GitHub remote or `gh` auth/availability is missing; merge confirmation uses GitHub match-head-commit and does not delete branches or use admin bypass. Force push, stash, discard, rebase, branch deletion, and worktree create/remove are out of scope. |
...
│   │   ├── knowledge-v2.json              # Cortex knowledge (v2) kill switch + settings
...
│   ├── knowledge/                         # Legacy knowledge base (still the active path with knowledgeV2 off; not deleted by migration)
...
│   ├── knowledge/entries/*.md             # Cortex knowledge (v2): per-entry files with provenance frontmatter (dark until knowledgeV2.enabled)
│   ├── knowledge/INDEX.md                 # Cortex knowledge (v2): generated, token-capped, the only surface injected into prompts
...
| `CURSOR_API_KEY` | — | Cursor SDK runtime API key (enables Composer 2.5 in specialist selectors; detached auth/connect errors stay inside the worker runtime and fail closed as worker failures) |
