...
| **Model-specific instructions** | `swarm/model-catalog-service.ts`, `swarm/model-overrides.ts` | Settings Models UI | Optional user-authored per-model prompt instructions injected into the manager prompt via `${MODEL_SPECIFIC_INSTRUCTIONS}`. Forge provides no built-in defaults. Instructions persist in `model-overrides.json`; custom prompts must include the placeholder to opt in. |
