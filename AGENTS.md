...
| **Provider usage monitoring** | `stats/provider-usage-service.ts` | `components/stats/sections/ProviderUsage.tsx` | OAuth-based subscription rate-limit monitoring for OpenAI Codex and Anthropic Claude. Shows 5-hour rolling and weekly usage windows with reset timers in the dashboard stats panel. Cached per-provider with 3-minute TTL. |
