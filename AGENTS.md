...
| **Message routing** | `swarm/message-router.ts`, `swarm/turn-ledger.ts`, `swarm/runtime/runtime-event-projector.ts` | Manager All view / worker transcript | `MessageRouter.resolve()` is the single choke point for render/route/drop decisions — visible-by-default for user-facing origin, each decision reason-coded (`render:*`/`route:*`/`deny:*`) and receipted. A terminal worker result is persisted in the manager session as a system `conversation_message` with `source: worker_report`, `terminal`, and `sourceWorkerId`; it is excluded from model context, hidden from the manager's focused Web view, and visible in manager All. Routine direct-web worker callbacks are internal manager decision turns: the manager dispositions the report, performs the smallest bounded acceptance check needed, and explicitly publishes only an accepted outcome or material blocker. Selecting a worker opens that worker's transcript and defaults its Web/All filter to All; the dormant Detailed flag does not expand visibility. A silent-turn notice or `worker_outcome_backstop` applies only to an independently eligible, explicit web-visible obligation—not to an ordinary worker callback. |
...
| **Working plans** | `swarm/planning/` | `components/chat/plan/`, chat header/composer dock | Builder managers have one always-available `update_plan` tool with the Codex-style full-snapshot schema: optional explanation plus ordered steps in `pending`, `in_progress`, or `completed` state, with at most one in progress. The current plan persists in per-session `plan.json`; before replacement or clear, its outgoing revision is appended to per-session `plan-history.ndjson` for offline review. A compact control above the composer keeps current progress in view. When a completed plan is replaced with a different plan, one frozen, collapsible summary is appended to the durable conversation timeline; ordinary revisions do not create cards. The latest revision is injected as bounded runtime-only context on manager turns and preserved through compaction so runtime replacement does not lose it. There are no plan IDs, task IDs, modes, worker links, per-revision chat receipts, settings toggle, or Collaboration surface. Clearing a conversation clears its current plan, while stop/archive preserves it and forks start without copying the live plan or revision-history file. |
