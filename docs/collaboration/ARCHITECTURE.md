...
- Opt-in `subscribe_inventory` is a local Builder WebSocket capability. On collaboration-server and Remote Projects sockets, Builder authorization runs first: requests denied by that gate may receive `COLLABORATION_COMMAND_NOT_ALLOWED`. Only authorized requests that reach the local-runtime capability check receive `INVENTORY_NOT_SUPPORTED`, including authenticated admins. It is not a viewed conversation, not channel state, and not stored in `collab_*` tables.
...
| Automatic Browser Host | One local Desktop host with private embedded/Chrome per-tab affinity for the selected normal local manager | The viewing machine's host is not forwarded; normal remote managers may expose tools but get `unavailable-host` without a host connected directly to that backend | No tools or host bridge |
...
| Passive inventory | Opt-in `subscribe_inventory` on the local Builder socket; correlated `inventory_snapshot` is the baseline and positive capability ack; `inventory_pong` is liveness only | Not a supported capability: unauthorized Builder requests may receive `COLLABORATION_COMMAND_NOT_ALLOWED`; authorized requests that reach the local-runtime check receive `INVENTORY_NOT_SUPPORTED` | Same rejection order as Remote Projects |
...
| Local Builder inventory (not collaboration) | `subscribe_inventory`, `inventory_snapshot`, `inventory_pong`; on collaboration-server/Remote Projects, Builder authorization runs first (`COLLABORATION_COMMAND_NOT_ALLOWED` when denied) and only authorized requests that reach the local-runtime check receive `INVENTORY_NOT_SUPPORTED`; old servers fail closed with `INVALID_COMMAND` and must not fall back to `subscribe` |
...
5. Selecting a remote profile/session routes chat and supported project HTTP/WS surfaces through that origin. Non-chat local-only surfaces continue to use the local backend. The Automatic Browser Host is not routed: embedded views/partitions and optional Chrome registration/relay/authority/IPC stay on the local Builder connection. Opt-in `subscribe_inventory` is also not routed: Builder authorization runs first, so unauthorized requests may receive `COLLABORATION_COMMAND_NOT_ALLOWED`, and only authorized requests that reach the local-runtime capability check receive `INVENTORY_NOT_SUPPORTED`. Remote Projects keep explicit conversation `subscribe` as the viewed-session contract. Collaboration channels use `collab_subscribe_channel`, not conversation `subscribe`. Ordinary `{ type: "ping" }` still replies with `ready` on non-inventory sockets as transport compatibility; it is not a shared view contract.
