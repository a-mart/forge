...
- Multi-backend UI keeps metadata for all configured backends but subscribes to detail for exactly one active channel/backend at a time.
- Builder-only surfaces stay out of collaboration unless explicitly designed for it.

## Builder vs Collaboration

| Area | Builder | Collaboration |
|------|---------|---------------|
| Runtime target | `builder` default | `collaboration-server` |
| Primary data scope | User's local Forge data dir | Dedicated collaboration data dir/volume |
| Profile/session model | Visible manager profiles and sessions | Hidden `_collaboration` system profile with session-backed channels |
| Auth | Local app auth/provider credentials | Local Better Auth accounts, `admin` and `member` roles |
| Settings target | Local backend | Selected collaboration backend |
| Provider auth | Local Builder auth files | Selected collaboration backend auth files |
| Specialists | Builder target space | Collaboration target space |
| Skills | Builder/global/project skill loading | Collaboration-selected global skill loading; no channel-local skill authoring in v1 |
| Project Agents | Supported | Builder-only unless separately designed |
| Codex sidecar | Supported where enabled | Builder-only |
| Phoenix observability | Builder-only | Excluded in v1 |
| Terminals | Builder UI surface | Hidden/local Builder UX; backend terminal routes may still exist |
...
| Shared protocol | `packages/protocol/src/collaboration.ts` |
...
The protocol source of truth is `packages/protocol/src/collaboration.ts`.
...
Roles are `admin` and `member`. The first admin is bootstrapped from `FORGE_ADMIN_EMAIL` and `FORGE_ADMIN_PASSWORD` on a fresh deployment. Admins can use member/invite management and admin settings routes. Members can use collaboration channels but not admin settings.
...
Missing `TargetSpace` is treated as Builder-only for legacy compatibility. Collaboration built-ins use `collab-` handles such as `collab-planner` and `collab-reviewer`. Runtime and UI rosters filter by target space.
...
Treat these as Builder-only unless a task explicitly designs collaboration support:
...
- Builder archive/source-control/file-browser assumptions that require local user workspace semantics

Backend routes for some Builder-only systems may still exist in a collaboration process. Do not assume route presence means the feature is exposed or supported in collaboration UX.
