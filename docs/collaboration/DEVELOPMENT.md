...
- `POST /api/chat-artifacts/read` is a narrower transcript-derived capability for an absolute path that an eligible assistant message actually presented as a Markdown link. It is not a general outside-CWD file API and must not be replaced with or folded into `/api/read-file`. On Windows, the presented path must additionally resolve within the presenting transcript owner's project/worktree CWD; an outside-workspace claim is denied with HTTP 403 / `path_outside_workspace`.
