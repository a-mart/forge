...
The artifact reader caps transcript scanning at 256 MiB and file content at 2 MiB. It rejects symlinks in every path component and verifies stable device/inode identity, size, and real path before and after reading so replacements fail closed. POSIX uses a no-follow open; Windows opens a handle without reading, verifies that handle against the prior no-symlink walk, and repeats path and handle identity checks around the read. `stable_identity_unsupported` remains HTTP 501 when stable identity cannot be established. The Windows path is implemented and covered through an injected-platform test seam, but native-Windows filesystem smoke validation remains required; do not describe it as platform-validated until that smoke passes. Preserve the typed error/status mapping when changing this path; do not collapse authorization, missing, conflict, size, unsupported, and internal failures into an ambiguous success or generic 404.
