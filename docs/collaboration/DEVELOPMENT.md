...
- `POST /api/chat-artifacts/read` is a transcript-derived capability for an absolute path that an eligible assistant message actually presented as a Markdown link. It is not a general file API and must not be replaced with or folded into `/api/read-file`. Unlike the workspace-scoped reader, an artifact may resolve anywhere on the local filesystem, including outside the transcript CWD, project CWD, and Git worktrees. The reader preserves path canonicalization, existence and regular-file checks, bounded reads, and stable identity verification; the presenting transcript link is the location-independent authority.

The direct HTTP chat-artifact body is `{ transcriptAgentId, messageId, path }`. The WebSocket `api_proxy` body is exactly `{ messageId, path }`; the proxy derives `transcriptAgentId` from the socket's subscribed agent and must not accept a caller-supplied owner. Authorization requires a unique user-visible assistant `conversation_message` in an active, non-archived Builder manager/worker transcript, and the canonical native absolute path must appear in that message's parsed link tokens. On Windows, a Markdown destination with exactly one leading slash before a drive path, such as `/T:/repos/project/report.md`, canonicalizes to `T:/repos/project/report.md`; UNC, doubled-slash, drive-relative, and alternate-data-stream forms remain ineligible. Plain text, image tokens, templated/placeholder paths, Collaboration-channel transcripts, internal workers, external threads, and archived owners are ineligible.

The artifact reader caps transcript scanning at 256 MiB and file content by type: text/JSON content at 2 MiB, images at 4 MiB, and PDF artifact preview/raw payload at 16 MiB. It rejects symlinks in every path component and verifies stable device/inode identity, size, and real path before and after reading so replacements fail closed. POSIX uses a no-follow open; Windows opens a handle without reading, verifies that handle against the prior no-symlink walk, and repeats path and handle identity checks around the read. `stable_identity_unsupported` remains HTTP 501 when stable identity cannot be established. The Windows path is implemented and covered through an injected-platform test seam, but native-Windows filesystem smoke validation remains required; do not describe it as platform-validated until that smoke passes. Preserve the typed error/status mapping when changing this path; do not collapse authorization, missing, conflict, size, unsupported, and internal failures into an ambiguous success or generic 404.
...
- Collaboration uses the same manager-facing task modes and execution policies with TargetSpace-filtered prompt/custom-specialist definitions. Legacy tiers, lenses, and `collab-*` handles remain compatibility rewrites.
