...
For local multi-backend UI testing, `docker-compose.yml` also includes `forge-collaboration-server-secondary` behind the `multi-backend-test` profile. It publishes `http://127.0.0.1:47388` by default, uses `FORGE_SECONDARY_COLLABORATION_BASE_URL`, sets `FORGE_COLLABORATION_AUTH_COOKIE_NAME=forge_collab_secondary_session` inside the secondary container, and bind-mounts separate persistent data at `./.forge-collaboration-data-secondary` so auth, channels, and runtime files stay isolated from the primary server. Start it with `docker compose --profile multi-backend-test up forge-collaboration-server-secondary`, or start both local servers with `docker compose --profile multi-backend-test up`.

A first boot must provide `FORGE_ADMIN_EMAIL` and `FORGE_ADMIN_PASSWORD` so the initial admin account can be created. For local `docker compose` use, set `FORGE_COLLABORATION_BASE_URL=http://127.0.0.1:47387` by default, and update it if you change `FORGE_PUBLIC_PORT`. The primary compose service passes `FORGE_COLLABORATION_AUTH_COOKIE_NAME` through with the default `forge_collab_session`. If you enable the secondary test service, keep `FORGE_SECONDARY_COLLABORATION_BASE_URL` aligned with `FORGE_SECONDARY_PUBLIC_PORT` when overriding its default `47388` host port. The secondary service intentionally uses a different auth cookie name so `127.0.0.1:47387` and `127.0.0.1:47388` can both stay signed in in one browser; override it with `FORGE_SECONDARY_COLLABORATION_AUTH_COOKIE_NAME` only if needed. The shipped `docker-compose.yml` now defaults `FORGE_COLLABORATION_TRUSTED_ORIGINS` to `http://127.0.0.1:47188,http://127.0.0.1:47189` so local Forge UI/Electron dev and local preview can reach the Docker collaboration server without extra setup. Use `127.0.0.1` consistently for local HTTP split deployments; mixing `localhost` and `127.0.0.1` makes the auth cookies cross-site and therefore requires HTTPS. Hosted deployments should set `FORGE_COLLABORATION_BASE_URL` to the public browser URL for the collaboration server, and `FORGE_COLLABORATION_TRUSTED_ORIGINS` should list any Builder/UI origins that are allowed to talk to it in split deployments.

`FORGE_COLLABORATION_AUTH_SECRET` is optional. Leave it unset to let the server generate and persist a local secret in the data directory. `FORGE_COLLABORATION_AUTH_COOKIE_NAME` is also optional and defaults to `forge_collab_session`; only set it when multiple collaboration servers share one browser cookie scope, such as local same-host multi-backend testing. When a custom name is set, Better Auth auxiliary cookies are namespaced from it (`<name>_session_data`, `<name>_dont_remember`) so sibling collaboration servers do not share `better-auth.*` cookies.
...
| `FORGE_SECONDARY_PUBLIC_PORT` / `FORGE_SECONDARY_COLLABORATION_BASE_URL` | Optional local-only secondary compose service settings for multi-backend UI testing. Defaults are `47388` and `http://127.0.0.1:47388`. |
| `FORGE_COLLABORATION_AUTH_SECRET` | Auth secret. Generated automatically if omitted. |
| `FORGE_COLLABORATION_AUTH_COOKIE_NAME` | Optional session cookie name. Defaults to `forge_collab_session`; use a distinct value only when multiple local collaboration servers share one browser cookie scope. Custom values also namespace Better Auth auxiliary cookies as `<name>_session_data` and `<name>_dont_remember`. |
