...
Legacy exact-CWD executable surfaces (`<cwd>/.forge/extensions/`, `<cwd>/.pi/extensions/`, and `<cwd>/.pi/settings.json`) are preserved only under the same executable trust policy. New repositories should prefer the repo-root `.forge/` layout above.
...
Changing trust or block state immediately evicts affected manager and worker runtimes so stale executable code cannot remain loaded. If a manager is active, Forge detaches and restarts the runtime without terminating the session.
