...
If the new model is not in Pi upstream yet, Forge still has a synthetic resolution bridge in the backend runtime path. Runtime construction flows through the runtime provider dispatch facade and provider-specific creators under `apps/backend/src/swarm/runtime/{acp,claude,pi}/`. The relevant helper is `resolveExactModel()` in `apps/backend/src/swarm/swarm-manager-utils.ts`, and the Pi runtime creation path uses it when a runtime needs an exact Pi model.
