...
OpenRouter models added by a user are persisted overlays, not checked-in catalog entries. They are manager-eligible only when live OpenRouter metadata verifies `supportsTools: true`, the per-model manager override is explicitly enabled with the exact `openrouter:<model-id>` key, and an OpenRouter credential is configured. The default is manager-off; no family or preset fallback is created. The same override key can store an optional `contextWindowCap` from the card's **Context window** **Apply**/**Reset** controls; a positive whole-number cap cannot raise capacity above the advertised window, and blank Apply or Reset removes only the cap. Manager-enabled and **Configure routing** remain independent. Removing the model deletes the whole override key; re-adding it starts manager-off with no cap. Caps apply when an agent next starts; running agents keep their current snapshot. When OpenRouter settings load, Forge automatically reconciles legacy rows against current live tool metadata; rows that cannot be matched or refreshed remain non-manager rows until a later retry succeeds. OpenRouter manager eligibility is separate from compaction, and retired OpenRouter IDs fail closed rather than being migrated across providers. Shared OpenRouter routing defaults and exact-model routing overrides persist in `shared/config/openrouter-models.json`; manager and context-cap overrides persist under `openrouter:<model-id>` in `shared/config/model-overrides.json`. Do not add a second overlay path or treat endpoint discovery as a catalog source of truth. See [OpenRouter routing and privacy](MODEL_CATALOG.md#openrouter-routing-and-privacy) and [Override semantics](MODEL_CATALOG.md#override-semantics).
