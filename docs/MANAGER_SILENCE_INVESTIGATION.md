...
- Callback signature: user-role `message` whose text starts `WORKER REPORT: status: done|partial|blocked` (canonical synthesized no-callback report format). `SYSTEM: ...` wrappers are legacy formats retained only for compatibility with Pi empty-turn detection, including older `SYSTEM: status:` and `SYSTEM: [workerCallback]` variants.
