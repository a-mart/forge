# Deferred Upstream Items — Historical Tracking Document

> **Historical artifact:** This ledger preserves decisions recorded during the March 2026 upstream review. Its original statuses are not the current roadmap. Forge now pins `@earendil-works/pi-coding-agent` 0.80.6 and ships native Claude SDK support through `@anthropic-ai/claude-agent-sdk` 0.2.92; see [Pi Extensions](../PI_EXTENSIONS.md) and [Provider Authentication](../CONFIGURATION.md#provider-authentication) for current behavior.

**Purpose:** Preserve the upstream features and changes evaluated or deferred at that point in time.

**Last updated:** 2026-07-13 (historical context added; original decisions retained)
...
- **Analysis:** The original internal analysis is no longer part of tracked documentation.
- **Revisit when:** Anthropic ships SDK-only features we need, or Pi intermediary becomes a bottleneck
- **Later outcome:** Native Claude SDK runtime support subsequently shipped; this entry remains as the original deferral record.
...
- **Analysis:** The in-progress internal analysis was not retained in tracked documentation.
