...
- xAI models require either an API key or OAuth in Settings → Authentication. Settings manages one direct, non-pooled xAI credential slot, so configuring one method replaces the other. `XAI_API_KEY` remains an environment fallback when no Settings-managed xAI credential is resolved; it is not a second account or promised failover from stored OAuth.
