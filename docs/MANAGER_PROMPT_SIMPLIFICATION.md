...
- Keep execution focused on the requested outcome; avoid unnecessary handoffs and verification loops.
...
- **Adaptive:** start directly and delegate when completion time, total cost, or necessary independent assurance improves after accounting for handoff, waiting, acceptance, and likely rework.
- **Hands-on:** retain direct investigation, implementation, and validation, including the critical path. Complexity, ambiguity, multiple files or steps, and an isolated worktree alone do not trigger delegation. Delegate for explicit user requests, unavailable required capabilities, or a concrete benefit from separable work.

Adaptive and Hands-on may use normal project tools for manager-owned outcomes. Delegate first must not use shell or browser actions as an indirect project-mutation path. Preserve effective ownership; reconsider it when evidence changes the tradeoff. See [Worker Delegation](SPECIALISTS.md) for the maintained work-mode guidance.
...
A screenshot can establish appearance but cannot prove an interaction it does not exercise. Worker-reported tests and screenshots are evidence, but the manager still makes the acceptance decision. Do not repeat the worker's investigation or turn acceptance into another implementation pass. When acceptance finds a blocker, request a focused fix from the owner, or explicitly reassign ownership when justified, and recheck the failed acceptance point. Direct manager implementation does not require a separate acceptance ceremony; required project checks still apply.

### Delegation boundary

Use one owner by default. The selected work mode decides whether a handoff is worthwhile; the roster selects the worker afterward. Workers need explicit relevant context and do not automatically inherit the manager conversation. Avoid overlapping implementation ownership and automatic reviewer waves.
...
The posture block owns only the manager-versus-worker preference. Builder managers receive the same compact Direct, Checklist, and Graph model and the active roster. Collaboration prompts must stay within their runtime capabilities and must not prescribe Builder-only planning, goal, or history tools. Tool parameters, graph state transitions, retry details, stable identifiers, and other invocation mechanics belong in tool descriptions and schemas. Runtime guards enforce deterministic delivery and missing required responses; the shared routing footer defines intentional silence with `NO_REPLY`.
