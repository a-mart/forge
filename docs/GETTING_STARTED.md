...
Forge Desktop provides one local Automatic Browser for the selected Builder manager. Select **Browser** in the desktop rail: embedded tabs render in Forge with navigation, viewport, transient screenshot, recording, and dock/pop-out controls, while Chrome-backed tabs stay in Chrome and appear as compact cards with **Show in Chrome**. Physical resize and recording are embedded-only. A trusted click, key press, scroll, or touch interrupts active agent control; Forge settles the in-flight command and requires a fresh snapshot before another mutation when the exact lease can safely remain attached-idle. Idle pointer movement is ignored, but a mismatched synthetic pointer move cancels an active sequence. Eligible page navigation is revalidated on the same target rather than silently migrating. **Take Control** is terminal human takeover. See [Browser automation](BROWSER_AUTOMATION.md) for lifecycle and troubleshooting details.
...
Open **Stats → Throughput** to compare manager and worker response throughput, daily model trends, model summaries, and recent calls. Filter by date range (including a custom range), project, role, provider/model, attribution, or specialist. Stats remains a local Builder surface even when a Remote Project is selected.

Response throughput is intentionally **Pi-runtime-only**: it records manager and worker model calls that run through Pi, rather than treating a provider name or every runtime as evidence of support. Other runtime paths do not add records. The rate is provider-final output tokens divided by the complete monotonic duration from request start to terminal completion. It does not include gaps between separate model calls, but it does include hidden reasoning, buffered tool calls, and batched deltas within that request. An agent-level Pi retry becomes a separate call; provider-internal retries and Codex WebSocket replays are not timed as separate rates.

**Show response throughput in conversations** in **Settings → General** is a browser-local preference and is off by default. It controls only `tok/s` presentation in manager and worker conversations; it does not affect Pi provider-final measurement, durable session records, or **Stats → Throughput** history.
...
Terminal measurements are saved with session history and scanned into the separate historical Throughput view; generations from before this feature was recorded do not appear. Reconnecting preserves the latest in-app exact header value while fresh lifecycle state is restored. Forks deliberately omit source throughput measurements, so a fork begins its own history. Existing JSONL is never rewritten: valid stored provider output and request duration are re-derived; records without a valid request duration are unmeasured.
...
For larger work with meaningful parallel streams, dependencies, user gates, retries, or synthesis, a manager may use `update_work_graph` instead. A graph is an optional executable form of the same plan state, not a second planning system. Forge dispatches ready non-decision nodes up to the graph's concurrency limit and normally uses the default roster specialist for each node's task type from the session's active delegation preset. A named alternative specialist is appropriate only when its current guidance clearly fits. A successful worker node waits for the manager to verify its result and call `accept_work_graph_node`; Forge then completes only that node and releases newly ready dependents. Decision nodes wait for user or manager input and never auto-dispatch. Small requests, one-worker delegations, and short checklists should not use a graph.
