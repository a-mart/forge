...
For larger work with meaningful parallel streams, dependencies, user gates, retries, or synthesis, a manager may use `update_work_graph` instead. A graph is an optional executable form of the same plan state, not a second planning system. Forge dispatches ready non-decision nodes up to the graph's concurrency limit and normally resolves `route: auto` from each node's behavior and the session's active delegation roster. A named route is appropriate only when its current guidance clearly fits. A successful worker node waits for manager acceptance before its dependents become ready. Decision nodes wait for user or manager input and never auto-dispatch. Small requests, one-worker delegations, and short checklists should not use a graph.
