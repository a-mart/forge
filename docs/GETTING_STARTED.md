...
The file browser opens supported text files directly in an editable CodeMirror surface on desktop, so there is no required Edit button for those files. Editable Markdown opens in source/editor mode by default instead of preview-first. The editor preserves syntax highlighting for common source formats, including Dockerfile, YAML, shell, JSON/JSONL, Markdown, TS/JS, CSS, HTML/XML/SVG. Non-editable files stay preview/read-only, and mobile remains read-only for file editing. The Files panel also has a scaffold action that can create a starter `.forge/` tree and README without overwriting existing files.

Saves use the file version that was loaded when the file was opened. If the file changed on disk, Forge shows a conflict instead of silently overwriting it. The conflict actions are **Reload from disk**, **Overwrite anyway**, and **Cancel**. Unsaved edits are guarded when you switch files, close the panel, switch sessions, move through Source Control transitions, or delete the open file or an ancestor folder. File-tree context menus can permanently delete files or folders after an AlertDialog confirmation. Folder deletion warns that contents are removed recursively. If a delete affects the selected/open file, Forge clears that selection, refreshes the file tree and content cache, and refreshes Source Control. In the desktop app, you can still open files in your external editor or use **Show in folder** to reveal the file in Finder or File Explorer. When Source Control has a selected worktree, Files can browse and edit that worktree without changing the chat session's working directory. Source Control refreshes after same-workspace file edits and deletes, and blocks same-worktree mutations while dirty file edits need a decision. Use the Chat rail item to jump back to the current manager/session chat, including from a worker route to the parent manager thread.
