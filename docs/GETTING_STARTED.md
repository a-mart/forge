...
External Chrome supports status, create/open, navigation, snapshot, click, type, press, scroll, evaluate, and wait. It does not support physical resize, recording, managed download events/artifacts/open, standalone physical capture/export controls, or dock/pop-out. Both hosts share session state in `browser.json`; completed recordings under `artifacts/browser/` are Managed Browser-only.
