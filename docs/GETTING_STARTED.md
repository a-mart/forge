...
Forge chooses the target automatically. When the Forge extension is enabled and authenticated, it grants profile-wide access to eligible ordinary web tabs. `browser_status` returns a bounded `eligibleTabs` inventory across ready authenticated profiles. Tabless `browser_open(reuseExistingTab: true)` (the default) selects the active or most recently accessed eligible tab without requiring Chrome or the operating system to be focused; passing an inventory `tabId` selects that exact tab. Non-open operations remain sticky. `reuseExistingTab: false`, or no eligible tab, may create an inactive neutral `about:blank` tab for one authorized initial navigation. An explicit Chrome-backed tab never migrates, and Chrome-internal or other restricted pages remain excluded by the platform capability.
...
For larger work with meaningful parallel streams, dependencies, user gates, retries, or synthesis, a manager may use `update_work_graph` instead. A graph is an optional executable form of the same plan state, not a second planning system. Forge dispatches ready non-decision nodes up to the graph's concurrency limit and normally uses each node behavior's baseline route from the session's active delegation roster. A named route is appropriate only when its current guidance clearly fits. A successful worker node waits for the manager to verify its result and call `accept_work_graph_node`; Forge then completes only that node and releases newly ready dependents. Decision nodes wait for user or manager input and never auto-dispatch. Small requests, one-worker delegations, and short checklists should not use a graph.
