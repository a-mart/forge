...
For larger work with meaningful parallel streams, dependencies, user gates, retries, or synthesis, a manager may use `update_work_graph` instead. A graph is an optional executable form of the same plan state, not a second planning system. Forge dispatches ready non-decision nodes up to the graph's concurrency limit and derives support or routine execution from node kind; Deep is reserved for an explicit risk-based override or a retry after a blocked attempt. A successful worker node waits for manager acceptance before its dependents become ready. Decision nodes wait for user or manager input and never auto-dispatch. Small requests, one-worker delegations, and short checklists should not use a graph.
