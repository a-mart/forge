...
For larger work with meaningful parallel streams, dependencies, user gates, retries, or synthesis, a manager may use `update_work_graph` instead. A graph is an optional executable form of the same plan state, not a second planning system. Forge dispatches ready non-decision nodes up to the graph's concurrency limit and normally uses the default specialist for each node's task type from the session's active roster. A named alternative specialist is appropriate only when its current guidance clearly fits. A successful worker node waits for the manager to verify its result and call `accept_work_graph_node`; Forge then completes only that node and releases newly ready dependents. Decision nodes wait for user or manager input and never auto-dispatch. Small requests, one-worker delegations, and short checklists should not use a graph.
