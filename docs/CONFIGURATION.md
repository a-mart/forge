...
| `XAI_API_KEY` | — | Environment fallback for xAI/Grok API-key authentication when no Settings-managed xAI credential is resolved; it is not a second account or automatic failover for stored OAuth. |
