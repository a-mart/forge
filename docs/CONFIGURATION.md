...
Builder managers also expose `update_work_graph` for substantial work that benefits from dependencies, gates, retries, concurrency, or fan-in. The graph is stored as an optional richer shape inside the same `plan.json` snapshot and projected into the ordinary plan steps for compatibility. `maxConcurrency` defaults to 4 and is bounded from 1 through 8. Forge dispatches ready non-decision nodes automatically, records attempt worker and routing metadata, changes successful attempts to `awaiting_review`, and waits for a manager graph revision marking the node completed before releasing dependents. Re-submitting a blocked node as pending creates a retry. Forge attempts to stop running nodes removed, cancelled, or reset to pending before dispatching replacement work; a stop error is logged and does not block replacement dispatch. There is no graph setting or separate graph data directory.
...
Native xAI defaults to `grok-4.5`. API-key auth exposes its `low`, `medium`, `high`, and `xhigh` reasoning levels. OAuth uses a bounded `low`/`medium`/`high` fallback until authenticated discovery returns account-specific metadata, which is then authoritative for available reasoning choices. Discovery can additionally expose exactly the OAuth-only entitlement models `grok-build` and `grok-composer-2.5-fast`; they appear only when the active account returns valid entries and are removed if discovery fails or the effective auth becomes an API key. The xAI model `grok-composer-2.5-fast` is not Cursor SDK's `composer-2.5`.
