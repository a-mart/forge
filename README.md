...
**Active Work Plans** — For substantial work, a manager can keep a session-scoped coordination plan visible in chat. The Active Work card shows the current plan, item status, and linked worker evidence; the chat header indicator can collapse or expand the card without changing the underlying plan. Work Plans are descriptive progress state, not workflow runners, and they are saved with the session so they can survive pauses, restarts, compaction, or model changes.
...
- **Active Work** — session-scoped Work Plans can appear as a card in chat and a collapsible header indicator for substantial manager-led work.
