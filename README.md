...
**Context doesn't die.** When Claude Code compacts, you get amnesia. Forge's default Summary compaction writes structured handoff files, retains the most recent context, and summarizes the rest. Experimental Fresh windows can instead continue from a checkpoint and retrieve older history with the agent-only `history` tool. Compact and Smart compact follow the session's effective policy. On Summary, manual Smart compact on an already-idle Pi-backed manager stays idle, while active or interrupted work resumes. Fresh rejects busy manual attempts until idle, and idle stays idle. Active compaction or context recovery also shows a violet pulsing `C` badge in the sidebar. Pin critical messages and they'll survive every compaction. The pin navigator in the chat header lets you jump directly to any pinned message. Conversations that have compacted 50+ times still maintain coherence.
...
On **Summary**, when a session reaches ~85% context capacity, Forge pauses and writes a structured markdown handoff file capturing the current operational state, then compacts. The compacted context retains the most recent ~20,000 tokens verbatim and summarizes everything older. The handoff file ensures no critical context is lost. **Fresh windows** is experimental and opt-in: it continues at native settled threshold or overflow boundaries rather than that ~85% trigger. Compact and Smart compact use a deterministic checkpoint instead of an LLM summary, skip the Smart handoff, and leave older history recoverable through the agent-only `history` tool (`sessions`, `search`, `read`). There is no human history drawer. Fresh is executable only on supported ordinary Pi Builder managers (OpenAI/Codex or Anthropic). Collaboration, Cortex/system, Cursor SDK, plugin/external threads, and workers cannot run it; workers inherit the owning manager.

While compaction or context recovery is active, the session row shows a violet pulsing `C` badge in the sidebar. Compact and Smart compact follow the saved policy at the next context transition. On Summary, if you trigger Smart compact manually on an already-idle Pi-backed manager, it stays idle afterward; if the manager is active, interrupted, or dispatch-pending, it resumes after compaction. On Fresh, a busy manual attempt is rejected until idle, and an idle manager stays idle. **Settings → General → Compaction** still controls the compaction model, reasoning, and timeout for supported Pi-backed OpenAI/Codex and Anthropic manager compaction runtimes only, not Cursor SDK, xAI/Grok, or user-added OpenRouter manager models; OpenRouter manager eligibility is a separate policy and OpenRouter models are not compaction choices. Context continuation (Summary vs Fresh) lives in **Project Settings → Context management**, with an optional session override beside Send.

You can pin up to 10 messages per session (user or assistant) by clicking the pin icon. Pinned messages are preserved through compaction — under Summary their full content is injected into the summary under a dedicated section, and under Fresh they are included in the checkpoint.
