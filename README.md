...
- **Browser automation** — Forge Desktop exposes one protocol-v2 Automatic Browser Host, not a Skill. It assigns sticky per-tab affinity privately: common operations can use Chrome when the optional pinned unpacked extension/native relay is ready, while physical resize and recording use the embedded Electron target. There is no host picker or persisted host preference. Browser-workspace tab creation requests a dedicated ungrouped Chrome tab; model `browser_open` permits focused reuse by default. Chrome-backed tabs stay in Chrome and use **Show in Chrome**; embedded tabs expose viewport, transient screenshot, recording, and dock/pop-out controls. Set up Chrome once per profile and Forge data directory under **Settings → Use Chrome with Forge**. The session's schema-v2 `browser.json` stores logical tab affinity with Chrome page identity redacted, and completed recordings under `artifacts/browser/` are embedded-only. The host stays on the local Builder connection and is never forwarded to Remote Projects or Collaboration. A remote normal manager can still have typed tools but gets `unavailable-host` without a host connected directly to its backend. See [Browser automation](docs/BROWSER_AUTOMATION.md).
...
Managers and workers have access to built-in skills. These are separate from Forge Desktop's [Automatic Browser Host](docs/BROWSER_AUTOMATION.md), which has no Skills toggle and remains local to Desktop. The legacy `agent-browser` workflow remains a Skill:
...
For local projects, runtime data stays on the local Forge instance under `~/.forge` (or `%LOCALAPPDATA%\forge` on Windows). Optional Chrome deployment/authentication/relay state is data-directory-scoped under `integrations/external-chrome/`; each session's schema-v2 `profiles/<profileId>/sessions/<sessionId>/browser.json` stores privacy-bounded logical tab affinity and contains no selected host. Remote Projects preserve the same server-authoritative model: their profile/session data stays in the remote instance's `FORGE_DATA_DIR`, and repository files stay in its server-hosted workspace; the client does not clone or synchronize them locally. Back up each authoritative Forge data directory and its workspace mounts. See [docs/CONFIGURATION.md](docs/CONFIGURATION.md) for the full data layout, [Browser automation](docs/BROWSER_AUTOMATION.md) for local setup and boundaries, and the [Remote Projects guide](docs/collaboration/REMOTE_PROJECTS.md) for the topology.
