...
- **File browser** — full repository file browser with click-to-open in your editor (configurable: VS Code, Cursor, etc.). When Source Control has a selected worktree, Files can browse that worktree without moving the active chat session or manager workspace.
- **Source Control** — review current changes, commit history, worktrees, and GitHub pull requests from the workspace rail. Branch actions support fetch, switch branch, create branch, and fast-forward-only pull with confirmation and preflight checks before any write. Pull Requests uses the `gh` CLI and shows unavailable or degraded states when there is no GitHub remote, `gh` is missing, or `gh` is not authenticated. PR merge requires confirmation, re-checks the head commit with GitHub's match-head-commit guard, and does not delete branches or use admin bypass. Force push, stash, discard, rebase, branch deletion, and worktree create/remove are intentionally out of scope.
