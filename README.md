...
- **Stats** — usage and activity summaries, Token Analytics, and a Throughput tab. Throughput records manager and worker model calls made through the Pi runtime. The browser-local, default-off **Settings → General → Show throughput in conversations** preference controls only the manager and worker conversation indicators; when enabled, they show only provider-final `tok/s` (or `—`), never an estimate. It does not affect measurement, durable records, or **Stats → Throughput** history, whose rates use qualifying provider-final output tokens and observed first-output-to-stream-end timing. Throughput is separate from overview and Token Analytics usage totals. Session totals are historical and include archived data, while active counts exclude archived projects and sessions. When Cursor SDK is used, its usage is included in dashboard stats, token analytics, and telemetry provider inference.
