...
- **Stats** — usage and activity summaries, Token Analytics, and a Throughput tab. Throughput records manager and worker model calls made through the Pi runtime; its stable manager and worker indicators show only provider-final `tok/s` (or `—`), never an estimate, while historical rates use qualifying provider-final output tokens and observed first-output-to-stream-end timing. It is separate from overview and Token Analytics usage totals. Session totals are historical and include archived data, while active counts exclude archived projects and sessions. When Cursor SDK is used, its usage is included in dashboard stats, token analytics, and telemetry provider inference.
