...
On first launch, configure at least one model provider in Settings to run agents. **Settings → Authentication** currently shows OAuth account-pool cards for OpenAI and Anthropic, plus key/token rows for xAI, OpenRouter, and Cursor SDK. Status and auth-type badges appear only on applicable cards. OpenAI/Codex can also use the Forge Auth broker in v1, which supplies short-lived leases that Forge acquires, renews, reports, and releases while runtimes are active. The primary broker setup path is a one-time setup link from your broker administrator: paste it in Settings → Authentication → OpenAI → Forge Auth broker, and Forge redeems it server-to-server without asking you to copy runtime tokens. While broker mode is active, local OpenAI credentials stay visible but are read-only. Claude SDK authentication is external to these rows: run `claude login` to use Claude Code CLI OAuth. Forge will walk you through a short welcome conversation to learn your preferences.
...
- Credentials for at least one supported agent provider (for example, OpenAI, Anthropic, xAI, OpenRouter, Cursor SDK, or Claude SDK via `claude login`). For OpenAI/Codex only, a Forge Auth broker setup link can configure broker-backed access instead of local OpenAI credentials.
...
- **Workspace rail** — the desktop app adds a left activity rail for Chat, Browser, Files, Source Control, Terminal, Cron/Schedules, and Artifacts/Dashboard. Chat returns to the current manager/session conversation, including from a selected worker route back to its parent manager thread. Browser opens the selected local Desktop browser host for the current manager: Forge-owned Managed Browser webviews or explicitly attached External Chrome tabs. Files opens in a left split pane beside the rail with a resizable tree and file surface for direct text editing or previews. Source Control opens inline with Changes, History, Worktrees, and Pull Requests tabs. Selecting a worktree changes the Source Control and Files browsing context without changing the chat session CWD. Browser, Files, Source Control, Artifacts/Dashboard, and Schedules switch mutually exclusively so panes do not stack or hide behind each other. Terminal stays independent and persistent instead of being part of that surface switch. Mobile keeps the header/drawer workspace controls.
- **Browser automation** — Managed Browser and External Chrome (Local Beta) are local Forge Desktop capabilities, not Skills. Choose the session's **Browser host** in the Browser workspace; changing that preference does not detach an existing External Chrome lease. Managed Browser owns Electron webviews and supports physical viewport controls, transient screenshots, recordings, and dock/pop-out. External Chrome uses a pinned unpacked extension plus native host to operate only tabs you explicitly attach in Chrome 125+; use a dedicated Chrome profile. External Chrome does not support physical resize, recording, managed download events/artifacts/open, standalone physical capture/export controls, or dock/pop-out. Set it up once per Chrome profile and Forge data directory by loading the exact stable folder shown in **Settings → External Chrome (Local Beta)**. Both hosts share the session's `browser.json`, but completed recordings under `artifacts/browser/` are Managed Browser-only. Both hosts stay on the local Builder connection and are never forwarded to Remote Projects or Collaboration. A remote normal manager can still have typed tools but gets `unavailable-host` without a host connected directly to its backend. See [Browser automation](docs/BROWSER_AUTOMATION.md).
...
Managers and workers have access to built-in skills. These are separate from the Forge Desktop [browser hosts](docs/BROWSER_AUTOMATION.md): Managed Browser and External Chrome have no Skills toggle and remain local to Desktop, while the legacy `agent-browser` workflow remains a Skill:
...
For local projects, runtime data stays on the local Forge instance under `~/.forge` (or `%LOCALAPPDATA%\forge` on Windows). External Chrome deployment/authentication/relay state is data-directory-scoped under `integrations/external-chrome/`; session host selection and privacy-bounded state for both Desktop browser hosts share `profiles/<profileId>/sessions/<sessionId>/browser.json`. Remote Projects preserve the same server-authoritative model: their profile/session data stays in the remote instance's `FORGE_DATA_DIR`, and repository files stay in its server-hosted workspace; the client does not clone or synchronize them locally. Back up each authoritative Forge data directory and its workspace mounts. See [docs/CONFIGURATION.md](docs/CONFIGURATION.md) for the full data layout, [Browser automation](docs/BROWSER_AUTOMATION.md) for browser-host setup and boundaries, and the [Remote Projects guide](docs/collaboration/REMOTE_PROJECTS.md) for the topology.
...
Provider auth for OpenAI, Anthropic, xAI, OpenRouter, and Cursor SDK is managed under **Settings → Authentication**. OpenAI and Anthropic use the current OAuth account-pool cards; xAI, OpenRouter, and Cursor SDK use masked key/token rows with provider links. Status and auth-type badges appear only on applicable cards. Claude SDK authentication is external to Settings and uses `claude login`; Cursor SDK can also use shared secrets or environment-based `CURSOR_API_KEY`. Cursor SDK Composer 2.5 and Cursor Grok 4.5 can appear in manager and specialist selectors when credentials and model visibility allow them. Background auth/transport failures are contained inside the worker runtime and report as worker failures rather than crashing the backend or Electron child. OpenAI/Codex Forge Auth broker and local-credential behavior is described in [docs/CONFIGURATION.md](docs/CONFIGURATION.md). The **Settings → Models** tab provides a full catalog of supported models with visibility controls and context window overrides. **Settings → Observability** configures the local Phoenix exporter and persists its settings under `shared/config/phoenix-observability.json`; live Phoenix/golden-trace validation and Electron package/preflight validation remain user-owned gates for this V1 feature.
