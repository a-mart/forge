...
- **File browser** — full repository file browser with direct single-file text editing on desktop. Supported text files open directly in a CodeMirror editor with syntax highlighting for common source formats, including Dockerfile, YAML, shell, JSON/JSONL, Markdown, TS/JS, CSS, HTML/XML/SVG. Editable Markdown opens in source/editor mode by default. Non-editable files stay preview/read-only, and mobile remains read-only for file editing. Saves are versioned with explicit conflict handling. Desktop file-tree context menus can permanently delete files or folders after confirmation; folder deletion warns that contents are removed recursively. You can still open files in your external editor (configurable: VS Code, Cursor, etc.). When Source Control has a selected worktree, Files can browse and edit that worktree without moving the active chat session or manager workspace.
- **Source Control** — review current changes, commit history, worktrees, and GitHub pull requests from the workspace rail. Forge may quietly auto-fetch stale origin data when you enter Source Control or change context, while manual Fetch stays explicit and reports errors. Branch actions support fetch, switch branch, create branch, and fast-forward-only pull with confirmation and preflight checks before any write. Same-workspace file edits refresh Source Control, and unsaved inline edits guard Source Control transitions or same-worktree mutations instead of being discarded. Pull Requests uses the `gh` CLI and shows unavailable or degraded states when there is no GitHub remote, `gh` is missing, or `gh` is not authenticated. PR merge requires confirmation, re-checks the head commit with GitHub's match-head-commit guard, and does not delete branches or use admin bypass. Force push, stash, discard, rebase, branch deletion, and worktree create/remove are intentionally out of scope.
