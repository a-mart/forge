...
- **Source Control** — review current changes, commit history, worktrees, and GitHub pull requests from the workspace rail. Branch actions support fetch, switch branch, create branch, and fast-forward-only pull with confirmation and preflight checks before any write. Same-workspace file edits refresh Source Control, and unsaved inline edits guard Source Control transitions or same-worktree mutations instead of being discarded. Pull Requests uses the `gh` CLI and shows unavailable or degraded states when there is no GitHub remote, `gh` is missing, or `gh` is not authenticated. PR merge requires confirmation, re-checks the head commit with GitHub's match-head-commit guard, and does not delete branches or use admin bypass. Force push, stash, discard, rebase, branch deletion, and worktree create/remove are intentionally out of scope.
