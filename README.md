...
- **Browser automation** — Forge Desktop's local Automatic Browser can render an embedded tab in Forge or keep a Chrome-backed tab in Chrome with **Show in Chrome**. Embedded tabs include viewport, transient screenshot, recording, and dock/pop-out controls; Chrome omits embedded-only controls. Set up optional Chrome use once per Chrome profile and Forge data directory under **Settings → Use Chrome with Forge**. The capability is not forwarded to Remote Projects or Collaboration. See [Browser automation](docs/BROWSER_AUTOMATION.md) for behavior, setup, persistence, and security boundaries.
