...
- **Stats** — usage and activity summaries, Token Analytics, and a Throughput tab. Throughput records manager and worker model calls made through the Pi runtime; live values are marked as estimates until provider-final usage arrives, and historical rates use qualifying final tokens plus streamed-output boundaries. It is separate from overview and Token Analytics usage totals. Session totals are historical and include archived data, while active counts exclude archived projects and sessions. When Cursor SDK is used, its usage is included in dashboard stats, token analytics, and telemetry provider inference.
