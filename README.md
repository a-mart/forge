...
- **Workspace rail** — the desktop app adds a left activity rail for Chat, Browser, Files, Source Control, Terminal, Cron/Schedules, and Artifacts/Dashboard. Chat returns to the current manager/session conversation, including from a selected worker route back to its parent manager thread. Browser opens Forge-owned webviews for the selected local manager. Files opens in a left split pane beside the rail with a resizable tree and file surface for direct text editing or previews. Source Control opens inline with Changes, History, Worktrees, and Pull Requests tabs. Selecting a worktree changes the Source Control and Files browsing context without changing the chat session CWD. Browser, Files, Source Control, Artifacts/Dashboard, and Schedules switch mutually exclusively so panes do not stack or hide behind each other. Terminal stays independent and persistent instead of being part of that surface switch. Mobile keeps the header/drawer workspace controls.
- **Managed Browser** — Managed Browser is a Forge Desktop capability, not a Skill. It controls Forge-owned Electron webviews for the selected local Builder manager. It does not attach to your everyday Chrome profile, and local browser IPC is not forwarded to Remote Projects or Collaboration channels. Use its tabs, navigation, zoom, viewport presets, screenshots, recordings, and shared human/agent control from the Browser rail item. Ordinary web clients show the workspace as unavailable, and a normal manager on a remote backend can receive the typed tools but gets `unavailable-host` without a Desktop host connected to that backend. See [Managed Browser](docs/BROWSER_AUTOMATION.md).
...
Managers and workers have access to built-in skills. These are separate from the Forge Desktop [Managed Browser](docs/BROWSER_AUTOMATION.md), which has no Skills toggle and never attaches to an everyday Chrome profile:
