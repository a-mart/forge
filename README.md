...
Remote Projects lets Builder show normal projects from an authenticated Forge collaboration server alongside local projects. Add and test the server under **Settings → Collaboration**, sign in, enable **Remote projects** for that connection, then select its blue, globe-marked project or session row in the unified Builder sidebar. Selecting a row changes the active origin for supported project surfaces: chat and agent execution, Files, Source Control, attachments, Session Audit, and terminals (when the server permits them) target the remote instance. Selecting a local row switches those surfaces back.
...
- **Unified local/remote sidebar** — enabled Remote Projects appear as blue, globe-marked project and session rows beside local projects. Remote rows are select-only and can show connecting, sign-in-required, unreachable, server-disabled, update-required, or empty states. Selecting one makes its server the active origin for supported project surfaces; author chips identify other users in remote chat, while the viewer indicator is presence for subscribed session viewers only—not typing, editing, or locking. The saved mixed order is a local preference and does not grant access to any remote project.
