...
**Working plans** — For substantial multi-step work, Builder managers can maintain a concise current checklist through `update_plan`. Plans use Pending, In progress, and Completed steps, support multiple concurrently active steps, and persist per session in `plan.json`. The first revision creates one card at that point in the conversation; later revisions update the same card without moving it, and completion freezes it there as a collapsed **Plan complete** card. A subsequent plan gets a new timeline card. Compact header and composer controls keep the current plan available when its card has scrolled away. Worker delegation tools accept an optional exact `planStep` association for lightweight token attribution. Assignment and completed-plan usage receipts are appended to the session's `plan-usage.ndjson` for offline analysis; there is no additional project-management surface.
