...
- **Workspace rail** — the desktop app adds a left activity rail for Files, Source Control, Terminal, Cron/Schedules, and Artifacts/Dashboard. Files opens in a left split pane beside the rail with a resizable tree and preview. Source Control opens inline with Changes, History, Worktrees, and Pull Requests tabs. Selecting a worktree changes the Source Control and Files browsing context without changing the chat session CWD. Mobile keeps the header/drawer workspace controls.
