...
**Working plans** — For substantial multi-step work, Builder managers can maintain a concise current checklist through `update_plan`. Plans use Pending, In progress, and Completed steps, support multiple concurrently active steps, persist per session in `plan.json`, and appear in the chat header and at the top of the transcript. Worker delegation tools accept an optional exact `planStep` association for lightweight token attribution. Completed step and plan usage receipts are appended to the session's `plan-usage.ndjson` for offline analysis; there is no additional UI or project-management surface.
