...
**Active Work Plans** — For substantial work, a manager can keep a session-scoped coordination plan visible in chat. The Active Work card shows the current plan, item status, and linked worker evidence; the chat header indicator can collapse or expand the card without changing the underlying plan. Creating a plan also writes a durable `work_plan_created` receipt into chat history, and completed prior plans can be expanded as read-only receipts from the chat timeline or the Active Work card overflow. Past receipts are view-only; they are not editable or reopenable as live plans. Work Plans are descriptive progress state, not workflow runners, and they are saved with the session so they can survive pauses, restarts, compaction, or model changes.
...
- **Worker pills and external threads** — green indicators show active workers. Click for a quick peek at what each worker is doing. Builder web also supports a leading `@Codex` or `[@Codex]` message to route text to an external Codex app-server sidecar thread. Codex sidecar threads are persisted by default and shown as worker-like external-thread cards; selecting the Codex sidecar and sending follow-up text continues that same sidecar thread.
- **Active Work** — session-scoped Work Plans can appear as a card in chat and a collapsible header indicator for substantial manager-led work. Creating a plan writes a durable receipt into chat history, and historical plan receipts stay in chat as expandable read-only entries.
...
| Active Work Plans | Guidance for concise durable Work Plans during substantial manager-led work | — |
...
| **Agents** | Manager and worker agents run through Forge's runtime facade, backed by Pi, Claude SDK, or Cursor SDK depending on the selected model/provider. Cursor SDK exposes Composer 2.5 for manager and specialist selectors, and background auth/transport failures stay contained inside the worker runtime so they surface as worker failures instead of crashing the backend or Electron child. Builder web can also route a leading `@Codex` / `[@Codex]` text message to an external Codex app-server sidecar thread. Sidecar display cards stay append-only in the parent session and are excluded from manager model context. Each Forge-managed worker runs in its own worktree. |
...
- **Agent Runtimes** — OpenAI Codex Responses transport defaults to `sse` for reliability when `FORGE_OPENAI_CODEX_TRANSPORT` is unset, blank, or invalid; `websocket-cached` remains an explicit experimental/canary opt-in via `FORGE_OPENAI_CODEX_TRANSPORT=websocket-cached`, with `websocket` and `auto` also supported. Sanitized transport diagnostics stay opt-in via `FORGE_CODEX_TRANSPORT_DEBUG=1` at `GET /api/debug/codex-transport`. The separate Builder web `@Codex` sidecar path requires the Codex CLI app-server, accepts text only, is excluded from Collaboration, and does not provide direct MCP support inside Forge.
