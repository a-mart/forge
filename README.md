...
**Working plans** — For substantial multi-step work, Builder managers can maintain a concise current checklist through `update_plan`. Plans use Pending, In progress, and Completed steps, persist per session in `plan.json`, and appear in the chat header and at the top of the transcript. They are always available in Builder and intentionally omit project-management features such as assignees, priorities, dependencies, and historical plan receipts.
