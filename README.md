...
**Context doesn't die.** When Claude Code compacts, you get amnesia. Forge's default Summary compaction writes structured handoff files, retains the most recent context, and summarizes the rest. Experimental Fresh windows can instead continue from a checkpoint and retrieve older history. Compact and Smart compact follow the session's effective policy. On Summary, manual Smart compact on an already-idle Pi-backed manager stays idle, while active or interrupted work resumes. Fresh rejects busy manual attempts until idle, and idle stays idle. Active compaction or context recovery also shows a violet pulsing `C` badge in the sidebar. Pin critical messages and they'll survive every compaction. The pin navigator in the chat header lets you jump directly to any pinned message. Conversations that have compacted 50+ times still maintain coherence.
...
When a session reaches ~85% context capacity, Forge continues according to the effective context policy. **Summary** remains the default: Forge pauses and writes a structured markdown handoff file capturing the current operational state, then compacts. The compacted context retains the most recent ~20,000 tokens verbatim and summarizes everything older. The handoff file ensures no critical context is lost. **Fresh windows** is experimental and opt-in: Compact and Smart compact use a deterministic checkpoint instead of an LLM summary, skip the Smart handoff, and leave older history recoverable through agent-only `history` search. Fresh is executable only on supported ordinary Pi Builder managers (OpenAI/Codex or Anthropic). Collaboration, Cortex/system, Cursor SDK, plugin/external threads, and workers cannot run it; workers inherit the owning manager.
