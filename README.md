...
Remote Projects lets Builder show normal projects from an authenticated Forge collaboration server alongside local projects. Add and test the server under **Settings → Collaboration**, sign in, enable **Remote projects** for that connection, then select its blue, globe-marked project header or a nested session row beneath it. Nested session rows use status dots rather than the globe marker. Selecting a row changes the active origin for supported project surfaces: chat and agent execution, Files, Source Control, attachments, Session Audit, and terminals target the remote instance. Selecting a local row switches those surfaces back.
...
**Active Work Plans (currently parked)** — Active Work Plans are currently unavailable. There is no Settings toggle, managers do not receive the `task` tool or Active Work guidance, and Forge does not show the live Active Work card/header or hydrate plans from task snapshots. Older `work_plan_created` receipts can still appear in chat as read-only records based on their creation snapshots; they are not active or resumable plans. Existing `tasks.json` sidecars and `work-plans.json` configuration files are preserved only as compatibility data.
...
- **Unified local/remote sidebar** — enabled remote project headers appear blue with a globe marker beside local projects; nested session rows sit beneath them and use status dots. Remote project actions are limited: **Change Working Directory** is available on the project header through the server directory browser, while local rename, archive, delete, fork, and model actions are absent. Connections can show connecting, sign-in-required, unreachable, server-disabled, update-required, or empty states. Selecting a remote project or session makes its server the active origin for supported project surfaces; author chips identify other users in remote chat, while the viewer indicator is presence for subscribed session viewers only—not typing, editing, or locking. The saved mixed order is a local preference and does not grant access to any remote project.
- **Workspace rail** — the desktop app adds a left activity rail for Chat, Files, Source Control, Terminal, Cron/Schedules, and Artifacts/Dashboard. Chat returns to the current manager/session conversation, including from a selected worker route back to its parent manager thread. Files opens in a left split pane beside the rail with a resizable tree and file surface for direct text editing or previews. Source Control opens inline with Changes, History, Worktrees, and Pull Requests tabs. Selecting a worktree changes the Source Control and Files browsing context without changing the chat session CWD. Files, Source Control, Artifacts/Dashboard, and Schedules switch mutually exclusively so panes do not stack or hide behind each other. Terminal stays independent and persistent instead of being part of that surface switch. Mobile keeps the header/drawer workspace controls.
...
- **Active Work (currently parked)** — the live card, header control, manager tool/guidance, and task-snapshot hydration are unavailable. Older `work_plan_created` receipts may still render as read-only creation snapshots.
