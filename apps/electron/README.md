...
- **Renderer process** — loads the staged UI bundle from `ui/index.html`, registers one Desktop browser host with the local Builder backend, projects canonical embedded-tab state, and renders either the embedded workspace or a Chrome-backed tab card; it receives no Chrome inventory or authority controls
