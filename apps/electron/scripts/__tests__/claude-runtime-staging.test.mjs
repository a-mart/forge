...
describe('Claude desktop runtime staging', () => {
  ...
  it('rejects missing and mismatched native packages in a standalone staged tree', async () => {
    ...
    try {
      ...
      await writeFile(path.join(sdk, 'package.json'), JSON.stringify({ name: '@anthropic-ai/claude-agent-sdk', version: '0.3.273' }))
      ...
      await writeFile(path.join(native, 'package.json'), JSON.stringify({ name: nativePackage, version: '0.3.273' }))
      expect(validateStagedClaudeSdkPackageDir(sdk)).toMatch(/Missing matched native/)
