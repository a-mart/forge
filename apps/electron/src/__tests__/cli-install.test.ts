...
describe('generatePosixShim (real)', () => {
  it('falls back when the hint exe is missing and execs with all arguments', () => {
    ...
    expect(generatePosixShim('/custom/path/hint', fallbackExe, fallbackCli)).toContain('/custom/path/hint')
  })
})

describe('generateWindowsCmdShim (real)', () => {
  it('falls back when the hint exe is stale, not just undefined', () => {
    ...
    expect(lines.slice(fallbackLabelIdx + 1).find((l) => l.startsWith('set "ELECTRON_EXE='))).toContain(fallbackExe)
    expect(shim).toContain(fallbackCli)
    expect(shim).toContain('%*')
  })
})
...
describe('generateWindowsPs1Shim (real)', () => {
  it('falls back when the hint exe is missing or not on disk', () => {
    const fallbackExe = 'C:\\Programs\\Forge\\Forge.exe'
    const shim = generateWindowsPs1Shim(
      'C:\\Users\\test\\.forge\\cli\\install-hint',
      fallbackExe,
      'C:\\Programs\\Forge\\resources\\cli\\cli.js',
    )
