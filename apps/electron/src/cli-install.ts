...
export const SHIM_NAME_POSIX = 'forge'
...
export function writeInstallHint(): void {
  ...
  try {
    ...
    const content = serializeInstallHint(hint)
...
/**
 * Serialize an install hint to the key=value line format used by shims.
 * @internal Exported for testing.
 */
export function serializeInstallHint(hint: InstallHint): string {
  return [
...
/**
 * Check whether a directory is present in the current PATH.
 * @internal Exported for testing.
 */
...
/**
 * Generate a POSIX shell shim that reads the install hint at runtime.
 * @internal Exported for testing.
 */
export function generatePosixShim(hintPath: string, fallbackElectronExe: string, fallbackCliResource: string): string {
  return [
    '#!/bin/sh',
    ...
    'fi',
    '',
    '# Fallback to known platform location if hint is missing or stale',
...
/**
 * Generate a Windows CMD shim that reads the install hint at runtime.
 *
 * Mirrors POSIX/PS1 fallback behavior: if the hint exists but points to a
 * stale/missing Electron exe, falls back to known platform install locations.
 * @internal Exported for testing.
 */
export function generateWindowsCmdShim(hintPath: string, fallbackElectronExe: string, fallbackCliResource: string): string {
  return [
    '@echo off',
    ...
    ')',
    '',
    'rem Fallback if hint is missing, empty, or points to stale/moved executable',
    'if not defined ELECTRON_EXE goto :use_fallback',
    'if not exist "%ELECTRON_EXE%" goto :use_fallback',
    'goto :after_fallback',
    '',
    ':use_fallback',
...
/**
 * Generate a Windows PowerShell shim that reads the install hint at runtime.
 * @internal Exported for testing.
 */
export function generateWindowsPs1Shim(hintPath: string, fallbackElectronExe: string, fallbackCliResource: string): string {
  return [
    '# Forge CLI shim — generated by Forge Desktop.',
    '# Re-run "Install CLI" from Settings > CLI Access to update.',
    '',
...
/**
 * Verify the installed CLI shim works by executing it with `--version`.
 *
...
export function verifyCliInstall(installedPath?: string): { ok: boolean; output: string } {
  ...
  try {
    ...
    if (process.platform === 'win32') {
      ...
      result = execFileSync('cmd.exe', ['/c', shimPath, '--version'], {
        timeout: 15_000,
        encoding: 'utf8',
        windowsHide: true,
      })
    } else {
      result = execFileSync(shimPath, ['--version'], {
        timeout: 15_000,
        encoding: 'utf8',
      })
    }
...
/**
 * Resolve the expected installed shim path for the current platform.
 */
function resolveInstalledShimPath(): string {
  const binDir = resolveShimBinDir()
  if (process.platform === 'win32') {
    return path.join(binDir, SHIM_NAME_WIN_CMD)
  }
  return path.join(binDir, SHIM_NAME_POSIX)
}
