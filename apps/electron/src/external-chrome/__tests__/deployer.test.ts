...
async function fixture(
  ...
  const native = Buffer.from(`native-${version}`)
  ...
  const payloadSha256 = sha256(Buffer.from(payload))
  ...
  const manifest = {
    ...
    extension: {
      ...
      minimumChromeVersion: '125', shellAbi: 1, shellSha256: sha256(Buffer.from(`${manifestJson}${shell}`)),
...
describe('ExternalChromeDeployer', () => {
  ...
  it('rejects concurrent deployment locks', async () => {
    ...
    await fs.writeFile(paths.lock, '99999999\n')
    const staleLock = new FileDeploymentLock(fs, () => false)
