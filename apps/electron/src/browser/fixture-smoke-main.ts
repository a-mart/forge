...
void app.whenReady().then(async () => {
  ...
  (async()=>{try{
    ...
    await bridge.registerWebview({tab,webContentsId:webview.getWebContentsId(),visible:false,created:true});
    const presentation=await bridge.setTabPresentation({tabId:'fixture-tab',visible:true,viewportSetting:{mode:'fill'},renderedViewport:{width:800,height:600,deviceScaleFactor:window.devicePixelRatio||1},hostGeneration:1,sessionRevision:1,sequence:1});
