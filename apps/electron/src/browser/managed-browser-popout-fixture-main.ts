...
void app.whenReady().then(async () => {
  ...
  const session: BrowserSessionSnapshot = { schemaVersion: 1, sessionAgentId: 'product-session', profileId: 'product-profile', hostingState: 'hosted', tabs: [tab], activeTabId: tab.tabId, defaultTabId: tab.tabId, panelVisible: true, recentActions: [], revision: 1, createdAt: now, updatedAt: now }
  const request = <T extends BrowserAutomationRequest['operation']>(operation: T, input: Extract<BrowserAutomationRequest, { operation: T }>['input']): BrowserAutomationRequest => ({ requestId: `product-${operation}-${Date.now()}`, hostKind: 'managed-electron', sessionAgentId: tab.sessionAgentId, profileId: tab.profileId, tabId: tab.tabId, hostId: 'product-host', hostGeneration: 5, deadlineAt: new Date(Date.now() + 10_000).toISOString(), artifactDirectory: null, operation, input } as BrowserAutomationRequest)
  let passed = false
