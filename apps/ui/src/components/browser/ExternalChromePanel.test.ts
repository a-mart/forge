...
describe('ExternalChromePanel', () => {
  ...
  it('states M4 snapshot/interaction support without claiming physical capture or deferred M3/M4 gates', async () => {
    ...
    expect(notice?.textContent).toContain('Snapshots return transient screenshot data')
    expect(notice?.textContent).toContain('no standalone screenshot toolbar/export workflow')
    expect(notice?.textContent).toContain('Resize, recordings, download handling or saved artifacts, opening downloaded files, and dock/pop-out are not available.')
