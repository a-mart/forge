...
export function ModeSwitch({ activeSurface, onSelectSurface, className }: ModeSwitchProps) {
  ...
  return (
    <div className={cn('inline-flex w-full rounded-[9px] border border-sidebar-border bg-sidebar-accent/40 p-[3px]', className)}>
      {(['builder', 'collab'] as const).map((surface) => {
        ...
        return (
          <button
            ...
            className={cn(
              'inline-flex h-7 flex-1 items-center justify-center gap-1.5 rounded-[7px] px-3 text-xs font-medium capitalize transition-colors',
              isActive
