...
OpenAI also has a Forge Auth broker mode. It is v1-scoped to OpenAI/Codex and uses a separate broker to issue short-lived leases. The primary setup path is to paste a one-time setup link from your administrator into the Forge Auth broker panel. Forge redeems the invite server-to-server, stores only the broker URL and runtime token needed for leases, and never echoes the invite secret or broker token in the UI. Manual broker URL/token setup remains available under advanced settings for older deployments.
...
If `FORGE_OPENAI_CODEX_AUTH_MODE` is set in the environment, it overrides the Settings value and disables invite paste in the UI. In `central_broker` mode, saved broker URL/token values are ignored and Forge uses `FORGE_OPENAI_AUTH_BROKER_URL`, `FORGE_OPENAI_AUTH_BROKER_TOKEN`, `FORGE_OPENAI_AUTH_BROKER_INSTANCE_ID`, and `FORGE_OPENAI_AUTH_BROKER_INSTANCE_LABEL` from the environment instead.
