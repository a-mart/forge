...
A global switch controls the default. Each eligible project can inherit that default or explicitly force monitoring on or off. Projects that are archived, removed, moved to a different working directory, or no longer valid local Git repositories are removed from monitoring eligibility.
...
Monitoring is not a replacement for Source Control. Use Source Control to inspect changes, fetch deliberately, switch branches, or pull. Git Monitoring never merges, checks out, or pushes for you.
