...
- **Skills** — browse and configure optional skill-backed capabilities such as Brave search, image generation, and the separate `agent-browser` CLI workflow. The Automatic Browser Host is local to Forge Desktop, not a Skill; optional Chrome setup lives under **Use Chrome with Forge**.
