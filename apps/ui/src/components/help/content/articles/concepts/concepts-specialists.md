Forge delegates work through rosters of complete specialists. Work mode decides whether the manager normally delegates. A roster specialist combines a task type and its instructions with the model, reasoning, use/avoid guidance, fallback, and escalation behavior used to run the assignment.
...
Configure this under **Settings → Delegation**. Project-level task-instruction overrides take precedence over global definitions; delegation preset selection resolves global → project → session.
