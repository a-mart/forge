...
Use **Session Audit Log** when you need the full persisted diagnostic view. It reads canonical manager and worker JSONL sources, lets you switch from the manager session log to a worker transcript, and exposes capped parsed/raw JSON previews with offsets and hidden reasons. Worker internals are visible only after explicitly selecting a worker source in that audit view, and audit rows are not added to normal chat or model context.
