...
There is no human history drawer. Pause or resume indexing from **Settings → History**; that pause is not a search enable toggle. Local Builder managers and ordinary workers recover compacted or older evidence with the agent-only `history` tool (`sessions`, `search`, `read`). Indexing starts autonomously after local Builder hydration and prefers recent sources. Discover matching sessions, then search the current session first (including associated workers), then the current project if needed. Targeted `sessionAgentId` or `profileId` searches are also valid. Every search outside the current project requires a nonempty reason and no approval prompt; `all_local` is a deliberate broad search, not the only cross-project path. Search is lexical (ranked terms, quoted phrases, prefixes, and code or path tokens), not embeddings. Reads use source-qualified references, including parts for long entries. Historical evidence is not current authority. Coverage is `building`, `ready`, `degraded`, or `unavailable`; incomplete catch-up or `building` coverage means a no-match is not proof of absence. Unreadable sources surface as `degraded` or `unavailable`.

Canonical JSONL remains the source of truth. `shared/cache/history-recall.db` is a rebuildable derived index, not read authority. Indexing clips each entry to 32,768 characters, readable JSONL rows stop at 1 MiB, and a `history` read is bounded to 20,000 characters across the entry and neighbors. Collaboration, Cortex/system, plugin, and external-thread runtimes do not get this tool. Remote Projects stay origin-scoped. Secure Sessions secrets are excluded.
