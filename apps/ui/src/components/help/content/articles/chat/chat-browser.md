...
Normal supported navigation stays on the same Chrome-backed tab. Forge may briefly retain internal control between nearby actions, but you do not attach or release anything yourself. A click, key press, touch, or wheel gesture from you interrupts agent control; DevTools or another debugger, a restricted target, a lost tab, a timeout, or shutdown can also interrupt it. Forge never silently migrates an explicit Chrome target or replays an action that may have changed the page; retry only after inspecting the current tab.
