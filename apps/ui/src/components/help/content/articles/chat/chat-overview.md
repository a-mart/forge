...
For diagnostics, use **Session Audit Log** from the chat header menu. It reads the persisted session history directly from canonical JSONL files. The manager source shows the manager session log; selecting a worker source shows that worker's transcript and internal tool activity. The list stays paginated with compact summaries and capped previews for performance. Select a row to open a full JSON detail pane with syntax highlighting, formatted/raw toggles, copy, and clear loading or truncation states. Rows include category/type filters, byte offsets, and hidden reasons. This audit surface is for inspection only and is not part of the normal chat transcript or model context.
