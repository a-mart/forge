...
Non-chat Settings, Stats, Archive, onboarding, Cortex, provider usage, and sidebar ordering remain local even while a remote project is selected. Managed Browser and External Chrome are also local-only Desktop hosts, not Skills: the viewing machine's webviews, extension relay, candidates, leases, and IPC are never forwarded to Remote Projects or Collaboration channels. A remote normal manager may still expose browser tools, but without a host connected directly to that remote backend they return `unavailable-host`.
