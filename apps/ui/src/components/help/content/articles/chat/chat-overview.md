...
Non-chat Settings, Stats, Archive, onboarding, Cortex, provider usage, and sidebar ordering remain local even while a remote project is selected. Managed Browser is also local-only: the viewing machine's Desktop webviews and IPC are not forwarded to Remote Projects or Collaboration channels. A remote normal manager may still expose browser tools, but without a host connected to that remote backend they return `unavailable-host`.
