...
**Show response throughput in conversations** in **Settings → General** is off by default. Turn it on to show conversation controls; turning it off hides only those controls, while **Stats → Response throughput** continues collecting and showing history.
...
## Response-throughput statistics

Open **Stats → Response throughput** for historical manager/worker totals, role comparison, daily model trends, model summaries, and recent calls. The view is local to Builder even while a Remote Project is selected. Filter by range (including a custom range), project, role, provider/model, attribution, or specialist.
...
Existing JSONL history is never rewritten. Forge re-derives response throughput from stored provider-final output and stored request duration; records without a valid request duration remain unmeasured. The v2 historical cache rejects old first-output-tail entries and rebuilds from durable records.

## History and forks

Terminal measurements are saved with the session history. Reconnecting keeps the latest in-app exact result anchored while fresh lifecycle state is restored; **Stats → Response throughput** scans durable records into its separate historical cache. Data begins with generations recorded after this feature was added.

Forked sessions deliberately omit the source session's response-throughput measurements, so a fork starts a new response-throughput history.
