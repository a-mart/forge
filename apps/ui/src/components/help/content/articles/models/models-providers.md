...
Existing checked-in curated models (`grok-4`, `grok-4-fast`, `grok-4.20-0309-reasoning`, and `grok-4.20-0309-non-reasoning`) remain available.

OAuth model discovery is dynamic and account-specific, so the two entitlement models and their reasoning choices may differ by account or disappear after a refresh. Authenticated discovery accepts only those exact approved IDs; existing checked-in xAI models remain available. Switching to API-key auth hides these models and rejects stale selections before a provider request. Losing entitlement or failing discovery removes them from refreshed catalog, projection, and selector results.
