...
Collab Settings also includes members and invites, plus password management, and an **AI Roles** panel for managing workspace roles used by channel/category AI Role selectors. Admins can manage members and invites, issue temporary-password resets that require a password change, and users can change their own password. If a collaboration session or socket gets invalidated by a lifecycle change, the public UI shows sign-in recovery instead of looping on reconnect or staying stuck on loading.

Channel and category settings include **AI Role** selectors. Categories define the default AI role for new channels, and channels can choose their own role when needed. Channel-specific guidance is labeled **Additional instructions**. The selectors use canonical role IDs and workspace defaults.
