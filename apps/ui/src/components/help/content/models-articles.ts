...
const modelsProviders: HelpArticle = {
  ...
  content: `Forge supports five AI providers. Each has a different set of models with different tradeoffs.
...
Cursor SDK uses \`CURSOR_API_KEY\` and exposes Composer 2.5 for specialist workers only. It is native to the Cursor runtime, and background auth/transport failures stay contained in the worker runtime and show up as worker failures, not app crashes.
