...
Right-click any session to access: Copy Path, Rename, Fork, Override Session Model, Use Project Default, Stop, Resume, Archive, Restore, Mark as Unread, Mute/Unmute, or Delete. The Main (default) session in each profile cannot be deleted or archived directly. Muting a session suppresses notification sounds while keeping the unread badge visible. Override Session Model opens a one-screen dialog for picking a concrete model, and Use Project Default clears the override so the session inherits the project default again. Every session, including the root session, can override the project default model or revert to inheriting it. Archived sessions are read-only and unavailable for chat, model, CWD, project-agent reference edits, and terminal use until restored. Restore can open the restored target immediately when requested.
...
3. **Proposal** — The architect drafts a configuration including handle, "when to use" blurb, and system prompt. The resulting agent is stored in a dedicated per-handle directory under \`profiles/<profileId>/project-agents/<handle>/\`.
