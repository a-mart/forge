...
interface PrivateValueTextareaProps extends Omit<
...
const PrivateValueTextarea = React.forwardRef<
  HTMLTextAreaElement,
  PrivateValueTextareaProps
>(({ className, onValueChange, style, value, ...props }, ref) => {
  ...
  return (
    <Textarea
      {...props}
      ref={textareaRef}
      data-slot="private-value-textarea"
      onChange={handleChange}
      onPaste={handlePaste}
      style={style}
      className={cn('field-sizing-fixed min-h-24 max-h-48 resize-y', className)}
    />
  )
})
PrivateValueTextarea.displayName = 'PrivateValueTextarea'
...
function replaceDisplayRange(rawValue: string, nextDisplayValue: string): string {
  ...
  while (
    ...
    && currentDisplayValue[currentSuffixLength - 1]
      === nextDisplayValue[nextSuffixLength - 1]
  ) {
