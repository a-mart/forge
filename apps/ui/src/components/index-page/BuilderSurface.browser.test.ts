...
describe('BuilderSurface browser reveal policy', () => {
  it('reveals only the selected local Electron session at the current generation regardless of metadata revision', () => {
    ...
    expect(shouldRevealBrowserPanel({ electronHostAvailable: true, selectedSessionAgentId: 'session-1', request, currentHostGeneration: 4, currentRevision: 9 })).toBe(true)
  })
})
