...
export async function fetchSlashCommands(wsUrl: string, managerId: string): Promise<SlashCommand[]> {
  const endpoint = resolveApiEndpoint(wsUrl, `/api/managers/${encodeURIComponent(managerId)}/slash-commands`)
...
export async function createSlashCommand(wsUrl: string, managerId: string, command: { name: string; prompt: string }): Promise<SlashCommand> {
  const endpoint = resolveApiEndpoint(wsUrl, `/api/managers/${encodeURIComponent(managerId)}/slash-commands`)
...
export async function updateSlashCommand(wsUrl: string, managerId: string, id: string, patch: { name?: string; prompt?: string }): Promise<SlashCommand> {
  const endpoint = resolveApiEndpoint(wsUrl, `/api/managers/${encodeURIComponent(managerId)}/slash-commands/${encodeURIComponent(id)}`)
...
export async function deleteSlashCommand(wsUrl: string, managerId: string, id: string): Promise<void> {
  const endpoint = resolveApiEndpoint(wsUrl, `/api/managers/${encodeURIComponent(managerId)}/slash-commands/${encodeURIComponent(id)}`)
