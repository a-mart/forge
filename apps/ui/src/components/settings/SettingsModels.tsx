...
function ModelCard({
  ...
  const saveEnabled = useCallback(
    ...
    [model.enabledByDefault, modelKey, onRefresh, wsUrl, onCardSaveStart, onCardSaveEnd],
  )
  ...
  const saveManagerEnabled = useCallback(
    ...
    [defaultManagerEnabledValue, modelKey, onRefresh, wsUrl, onCardSaveStart, onCardSaveEnd],
  )
  ...
  }, [contextCapDraft, model.contextWindow, modelKey, onRefresh, wsUrl, onCardSaveStart, onCardSaveEnd])
  ...
  }, [builtInInstructions, instructionsDraft, modelKey, onRefresh, wsUrl, onCardSaveStart, onCardSaveEnd])
  ...
  }, [builtInInstructions, modelKey, onRefresh, wsUrl, onCardSaveStart, onCardSaveEnd])
  ...
  const resetEnabled = useCallback(async () => {
    ...
    onCardSaveStart?.(modelKey)
    try {
      await updateModelOverride(wsUrl, modelKey, { enabled: null })
      await onRefresh()
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : String(saveError))
    } finally {
  ...
  }, [modelKey, onRefresh, wsUrl, onCardSaveStart, onCardSaveEnd])
  ...
  const resetManagerEnabled = useCallback(async () => {
    ...
    onCardSaveStart?.(modelKey)
    try {
      await updateModelOverride(wsUrl, modelKey, { managerEnabled: null })
      await onRefresh()
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : String(saveError))
    } finally {
  ...
  const resetContextCap = useCallback(async () => {
    ...
    onCardSaveStart?.(modelKey)
    try {
      await updateModelOverride(wsUrl, modelKey, { contextWindowCap: null })
      await onRefresh()
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : String(saveError))
    } finally {
