...
describe('SettingsExternalChrome', () => {
  it('shows the no-store/security/profile/auto-reload ceremony and copies only coordinator-projected values', async () => {
    ...
    expect(document.body.textContent).toContain('not used by current Local Beta code')
    expect(document.body.textContent).toContain('managed download events/artifacts')
