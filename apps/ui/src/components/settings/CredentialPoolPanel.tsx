          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                aria-label={credential.isPrimary ? 'Primary account' : 'Set as primary'}
          ...
          {isEditing ? (
            <div className="flex items-center gap-1.5">
              <Input
                ...
                autoFocus
              />
              <Button type="button" variant="ghost" size="icon" className="size-6" aria-label="Confirm rename" onClick={commitRename}>
...
export interface CredentialPoolPanelProps {
  /** Provider key used for API calls (e.g. 'openai-codex', 'anthropic') */
  provider: string
  /** Human-readable provider name for UI labels (e.g. 'OpenAI', 'Anthropic') */
...
export function CredentialPoolPanel({
  ...
  const metadata = SETTINGS_AUTH_PROVIDER_META[provider as SettingsAuthProviderId] as
    | (typeof SETTINGS_AUTH_PROVIDER_META)[SettingsAuthProviderId]
    | undefined
  const displayDescription = description ?? metadata?.description ?? `${providerLabel} credential pool.`
      ...
      await startPoolAddAccountOAuthStream(
        ...
        provider,
        {
          ...
          onComplete: () => {
            ...
            onSuccess(`${providerLabel} account added to the pool.`)
          },
          onError: (message) => {
            setOauthFlow((prev) => ({
              ...prev,
    ...
    return (
      <div className="rounded-lg border border-border bg-card/50 p-4">
        <div className="flex items-center gap-2">
          <p className="text-[13px] font-semibold text-foreground">{providerLabel}</p>
        </div>
