...
- **Bounded download/egress:** after a token is verified but before any R2 read, the Durable Object reserves per-share download count and byte-egress budget using stored share byte metadata. The reservation is committed only after the R2 object is read and parsed successfully, or rolled back on R2 read/parse failure. Reservation cleanup is keyed by share; a one-time legacy-key migration rolls back stale legacy reservations and re-keys live ones with commit/rollback aliases before returning to share-scoped cleanup. Defaults are 20 downloads or 250 MiB per share.
