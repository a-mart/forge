...
describe('pinned offline identity and MV3 ledger', () => {
  ...
  it('ships accurate setup, attach, detach, and pending-lease side-panel copy', async () => {
    ...
    expect(html).toContain('Detach leaves those tabs open')
    ...
    expect(html).toContain('managed download events/artifacts/open')
    expect(html).toContain('standalone physical capture/export controls')
