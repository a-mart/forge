...
let controlState: 'human' | 'agent' | 'detached' = 'human'
...
function statusSvg(state: typeof controlState): string {
  const color = state === 'agent' ? '#5b5cf0' : state === 'detached' ? '#777777' : '#48b77b'
...
function setState(state: typeof controlState): void {
  ...
  element.style.boxShadow = `0 0 0 2px ${state === 'agent' ? '#5b5cf0' : '#48b77b'}`
...
port.onMessage.addListener((message) => {
  ...
  } else if (command.type === 'status' && (command.state === 'human' || command.state === 'agent' || command.state === 'detached')) {
