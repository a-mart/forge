...
type ControlState = 'human' | 'agent' | 'attached-idle' | 'detached'

installSingletonContentBridge(globalThis as unknown as Record<string, unknown>, (disposeSingleton) => {
  ...
  function statusSvg(state: ControlState): string {
    const color = state === 'agent' ? '#5b5cf0' : state === 'attached-idle' ? '#d69e2e' : state === 'detached' ? '#777777' : '#48b77b'
  ...
  function markFavicon(state: ControlState): void {
  ...
  function setState(state: ControlState): void {
    controlState = state
    if (state === 'detached') {
    ...
    element.style.boxShadow = `0 0 0 2px ${state === 'agent' ? '#5b5cf0' : state === 'attached-idle' ? '#d69e2e' : '#48b77b'}`
