...
export function createChatArtifactRoutes(options: {
  ...
  return [{ methods: METHODS, matches: pathname => pathname === PATH, handle: async (request, response) => {
    ...
    try { const body = await parseJsonBody(request, 64 * 1024); const result = await readPresentedChatArtifact(options.swarmManager, body as any, { securityPlatform: options.artifactSecurityPlatform }); sendJson(response, 200, result); }
    catch (error) {
