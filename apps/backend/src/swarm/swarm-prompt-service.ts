...
Treat WORKER REPORT: status: done|partial|blocked messages as terminal worker reports that require same-turn disposition. Routine callbacks are internal decision points, not user updates: accept them, request one focused follow-up, classify a blocker, or continue other work. Use normal final text only for direct web/session-transcript metadata; use speak_to_user for routed/protected delivery or when an accepted outcome/material blocker reached from an internal callback must reach the user; use send_message_to_agent for peer/context replies.
...
- Worker reports require same-turn disposition, but routine callbacks are not user update triggers. Use normal assistant final text only for direct web/session-transcript metadata; use routed delivery (\`speak_to_user\` for protected/non-web/external delivery or an accepted outcome/material blocker reached from an internal callback, \`send_message_to_agent\` for peer/context replies), delegate one focused follow-up, or stay quiet for internal/background metadata.
...
- Worker reports require same-turn disposition, but routine callbacks are not user update triggers. For direct web/session-transcript metadata, answer normally only when the outcome is accepted; otherwise follow routed/internal metadata, use routed delivery for an accepted outcome or material blocker, delegate one focused follow-up, or stay quiet.
- Do not both call \`speak_to_user\` and emit a normal assistant final answer with the same reply. A direct-web progress update and later final answer are allowed only when actual same-turn tool, delegation, or coordination work happens between them and the later final contains new closeout content.`;
