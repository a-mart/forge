...
describe("SwarmSettingsService.updateManagerModel", () => {
  it("preserves Secure Session authority while recycling the model runtime", async () => {
    ...
    await service.updateSessionModel("manager", "override", "pi-6", "high");
    ...
    expect(session.model).toMatchObject({ modelId: "gpt-6-astra", thinkingLevel: "high" });
  ...
  it("writes continuity requests before mutating models for both active and inactive sessions", async () => {
    ...
    await service.updateSessionModel(activeSession.agentId, "override", "pi-6");
    await service.updateSessionModel(inactiveSession.agentId, "override", "pi-6");

    expect(activeSession.model).toEqual(resolveModelDescriptorFromPreset("pi-6"));
    expect(inactiveSession.model).toEqual(resolveModelDescriptorFromPreset("pi-6"));
    expect(applyManagerRuntimeRecyclePolicy.mock.calls).toEqual([
    ...
    expect(activeState.requests[0]).toMatchObject({
      ...
      targetModel: resolveModelDescriptorFromPreset("pi-6")
    });
  ...
  it("does not emit notices or change any existing session when updating a project default", async () => {
    ...
    const inheritedNoop = createSession(root, "manager--noop", resolveModelDescriptorFromPreset("pi-6"));
    ...
    await service.updateProfileDefaultModel("manager", "pi-6");
    ...
    expect(inheritedNoop.model).toEqual(resolveModelDescriptorFromPreset("pi-6"));
  ...
  it("emits the accepted-change notice before a deferred or failed recycle", async () => {
    ...
    await service.updateSessionModel(deferred.agentId, "override", "pi-6");
    await service.updateSessionModel(failed.agentId, "override", "pi-6");
    ...
    expect(deferred.model).toEqual(resolveModelDescriptorFromPreset("pi-6"));
    expect(failed.model).toEqual(resolveModelDescriptorFromPreset("pi-6"));
  });
  ...
  it("does not emit a notice when descriptor persistence fails", async () => {
    ...
    await expect(service.updateManagerModel("manager", "pi-6")).rejects.toThrow("descriptor save failed");
  ...
  it("legacy session-targeted updateManagerModel creates sticky same-as-default overrides without recycling", async () => {
    const root = await createTempRoot();
    const defaultModel = resolveModelDescriptorFromPreset("pi-6");
    ...
    await service.updateManagerModel(session.agentId, "pi-6");
  ...
  it("aborts an explicit session model change when its continuity request cannot be written", async () => {
    ...
    await expect(service.updateSessionModel(secondSession.agentId, "override", "pi-6")).rejects.toThrow(/invalid session header/i);
  ...
  it("updates only the targeted session when called with a session agent id", async () => {
    ...
    await service.updateManagerModel(secondSession.agentId, "pi-6");

    expect(firstSession.model).toEqual(resolveModelDescriptorFromPreset("pi-codex"));
    expect(secondSession.model).toEqual(resolveModelDescriptorFromPreset("pi-6"));
    expect(applyManagerRuntimeRecyclePolicy.mock.calls).toEqual([
  ...
  it("uses the original pending source model when a busy session is changed twice before replacement attaches", async () => {
    ...
    await service.updateSessionModel("manager", "override", "pi-6");
    expect(session.model).toEqual(resolveModelDescriptorFromPreset("pi-6"));
  ...
  it("updates the project default without reading even malformed existing histories", async () => {
    ...
    expect(profiles.get("manager")).toMatchObject({
      defaultModel: resolveModelDescriptorFromPreset("pi-6"),
  ...
  it("rolls back session state and skips recycle when saveStore fails during a session model update", async () => {
    ...
    await expect(service.updateSessionModel(session.agentId, "override", "pi-6")).rejects.toThrow("save failed");
  ...
  it("rolls back profile and session state and skips recycle when saveStore fails during a profile default update", async () => {
    ...
    await expect(service.updateProfileDefaultModel("manager", "pi-6")).rejects.toThrow("save failed");
  ...
  it("treats session recycle failures after save as deferred and still completes the update", async () => {
    ...
    await expect(service.updateSessionModel(session.agentId, "override", "pi-6")).resolves.toBeUndefined();
  ...
  it("does not invoke runtime recycling when changing the project default", async () => {
    ...
    await expect(service.updateProfileDefaultModel("manager", "pi-6")).resolves.toBeUndefined();
