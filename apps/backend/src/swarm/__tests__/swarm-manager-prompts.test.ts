...
describe('SwarmManager', () => {
  ...
  it('requestProjectAgentRecommendations analyzes against the base manager prompt without reusing the old override', async () => {
    ...
    try {
      ...
      expect(options.currentSystemPrompt).toContain('server metadata indicates direct web/session-transcript user-facing closeout')
      expect(options.currentSystemPrompt).toContain('`speak_to_user` for protected/non-web/external/proactive user-facing closeouts')
  ...
  it('uses manager and default worker prompts with explicit visibility guidance', async () => {
    ...
    expect(managerPrompt).toContain('Use speak_to_user only for explicit routed delivery: non-web/external targets and routed/protected worker-report closeouts. Do not use it for normal final web replies or direct-web progress updates.')
