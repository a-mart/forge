...
describe('SwarmManager', () => {
  ...
  it('requestProjectAgentRecommendations analyzes against the base manager prompt without reusing the old override', async () => {
    ...
    try {
      ...
      expect(options.currentSystemPrompt).toContain('routine callbacks are not user update triggers')
      expect(options.currentSystemPrompt).toContain('an accepted outcome/material blocker reached from an internal callback')
  ...
  it('uses manager and default worker prompts with explicit visibility guidance', async () => {
    ...
    expect(managerPrompt).toContain('Use speak_to_user for explicit routed delivery or for an accepted outcome/material blocker reached from an internal worker callback. Do not use it for ordinary direct-web replies initiated by the user\'s current message.')
