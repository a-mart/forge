...
describe('SwarmManager', () => {
  ...
  it('requestProjectAgentRecommendations analyzes against the base manager prompt without reusing the old override', async () => {
    ...
    try {
      ...
      expect(options.currentSystemPrompt).toContain('Worker reports require same-turn disposition')
      expect(options.currentSystemPrompt).toContain('routine callbacks are not automatic user update triggers')
      expect(options.currentSystemPrompt).toContain('even when callback metadata is internal; otherwise end with exactly `NO_REPLY`')
  ...
  it('uses manager and default worker prompts with explicit visibility guidance', async () => {
    ...
    expect(managerPrompt).toContain('Use `speak_to_user` only for explicit routed/protected, non-web, or proactive external delivery. Do not use it merely because a normal Builder turn came from a worker callback.')
