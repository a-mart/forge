...
describe('SwarmManager', () => {
  ...
  it('requestProjectAgentRecommendations analyzes against the base manager prompt without reusing the old override', async () => {
    ...
    try {
      ...
      expect(options.currentSystemPrompt).toContain('Direct web/session progress before continuing work: use brief assistant text only when immediately followed by same-turn tool, delegation, or coordination work.')
      expect(options.currentSystemPrompt).toContain('Worker results require same-turn disposition')
      expect(options.currentSystemPrompt).toContain('they are not automatic user update triggers')
      expect(options.currentSystemPrompt).toContain('otherwise end with exactly `NO_REPLY`')
      expect(options.currentSystemPrompt).toContain('After `speak_to_user` fully delivers a response, end the provider cycle with exactly `NO_REPLY`')
  ...
  it('uses manager and default worker prompts with explicit visibility guidance', async () => {
    ...
    expect(managerPrompt).toContain('Normal web/session chat does not need a tool for final replies or same-turn progress updates.')
    expect(managerPrompt).toContain('Normal web/session chat final replies: just answer normally with final assistant text.')
    expect(managerPrompt).toContain('Do not call `speak_to_user` for these normal web replies.')
    expect(managerPrompt).toContain('Direct web/session progress before continuing work: write at most 1-2 sentences as assistant text only when you also start the next tool, delegation, or coordination action in the same turn.')
    expect(managerPrompt).toContain('Use `speak_to_user` only when explicit delivery to the current web session is required. Do not use it merely because a normal Builder turn came from a worker result.')
    expect(managerPrompt).toContain('After `speak_to_user` has fully delivered the response, end the provider cycle with exactly `NO_REPLY`')
    expect(managerPrompt).toContain('Never use `NO_REPLY` to avoid answering a direct user request that has not already received a visible response.')
    expect(managerPrompt).toContain('Delegate execution, not accountability.')
