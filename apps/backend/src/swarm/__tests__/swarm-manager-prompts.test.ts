...
describe('SwarmManager', () => {
  ...
  it('requestProjectAgentRecommendations analyzes against the base manager prompt without reusing the old override', async () => {
    ...
    try {
      ...
      expect(options.currentSystemPrompt).toContain('Direct web request or accepted closeout: answer with normal final assistant text.')
      expect(options.currentSystemPrompt).toContain('Direct progress: use brief assistant text only when an action follows in the same turn.')
      expect(options.currentSystemPrompt).toContain('Routed, protected, non-web, or proactive publication: use `speak_to_user`, then end with exactly `NO_REPLY` unless distinct new closeout content remains.')
      expect(options.currentSystemPrompt).toContain('terminal worker results requiring same-turn disposition, not automatic user updates')
      expect(options.currentSystemPrompt).toContain('Never duplicate one reply through `speak_to_user` and normal final text.')
  ...
  it('uses manager and default worker prompts with explicit visibility guidance', async () => {
    ...
    expect(managerPrompt).toContain('Direct web/session request or accepted closeout: answer with normal final assistant text.')
    expect(managerPrompt).toContain('Direct progress before more work: use at most 1-2 sentences only when a tool, delegation, or coordination action follows in the same turn.')
    expect(managerPrompt).toContain('Explicit routed or proactive publication: use `speak_to_user`, then end with exactly `NO_REPLY` unless there is distinct new closeout content.')
    expect(managerPrompt).toContain('Never duplicate one reply through both `speak_to_user` and normal final text.')
    expect(managerPrompt).toContain('Never use `NO_REPLY` to skip an unanswered direct user request.')
    expect(managerPrompt).toContain('When you delegate, delegate execution, not accountability.')
    expect(managerPrompt).toContain('Before substantive execution, whether direct or delegated, silently establish three things')
    expect(managerPrompt).toContain('Default review budget: one implementation pass, one bounded manager acceptance pass')
    expect(managerPrompt).toContain("A worker's `done` status, test count, screenshot, or review opinion is evidence, not acceptance by itself.")
    expect(managerPrompt).toContain('Your posture is **Delegation-first**.')
    expect(managerPrompt).toContain('Manager direct project work is read-only.')
