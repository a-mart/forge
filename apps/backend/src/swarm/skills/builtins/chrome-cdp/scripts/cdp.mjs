...
function matchesPolicy(url) {
  const allowPatterns = parsePolicyPatterns(process.env.CDP_URL_ALLOW);
  const blockPatterns = parsePolicyPatterns(process.env.CDP_URL_BLOCK);

  if (allowPatterns.length > 0 && !matchesAnyPattern(url, allowPatterns)) return false;
  if (blockPatterns.length > 0 && matchesAnyPattern(url, blockPatterns)) return false;
  return true;
}

function matchesContextPolicy(page) {
  const contextId = process.env.CDP_CONTEXT_ID?.trim();
  if (!contextId) return true;
  return (page.browserContextId || '') === contextId;
}

function isPageAllowed(page) {
  return matchesContextPolicy(page) && matchesPolicy(page.url || '');
}
...
async function navStr(cdp, sid, url) {
  ...
  if (!matchesPolicy(url)) {
    throw new Error('Navigation to URL blocked by policy');
  }
...
async function main() {
  ...
  if (cmd === 'open') {
    ...
    if (!matchesPolicy(url)) {
      console.error('URL blocked by CDP_URL_BLOCK/CDP_URL_ALLOW policy');
      process.exit(1);
    }
  ...
  const allowedPages = pages.filter(isPageAllowed);
  if (!allowedPages.some(page => page.targetId === targetId)) {
    console.error('Target not in allowed page list — access denied by policy');
    process.exit(1);
  }
