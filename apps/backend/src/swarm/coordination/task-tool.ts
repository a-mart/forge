...
export const taskToolSchema = Type.Object(
  ...
  },
  {
    additionalProperties: false,
    description:
      'Arguments for the manager-only task tool. This tool records coordination state only; task updates do not by themselves close final/blocked worker callbacks when user/peer closeout, choices, or delegation is needed. Provider-facing upsert_plan supports top-level plan fields plus create-time itemsText; it does not expose structured items arrays. Call `task.get` to retrieve the full current snapshot; successful mutations return compact acknowledgements. Recoverable conflicts return ok:false with an error code and suggestedAction; treat those as tool results, not fatal tool errors. Example: {"action":"upsert_plan","title":"Investigate bug","itemsText":"[active] Trace failure\\n[todo] Patch shared state"}.',
  },
)
