...
11. **Watermark**: Update `meta.json` review watermarks only after successful writes or a validated no-op outcome: `cortexReviewedBytes`, `cortexReviewedAt`, `cortexReviewedMemoryBytes`, `cortexReviewedMemoryAt`, `cortexReviewedFeedbackBytes`, `cortexReviewedFeedbackAt`. `cortexReviewedBytes` and scan `sliceStartBytes` are raw `session.jsonl` byte offsets for worker slicing; actionability comes from `reviewableTranscriptDeltaBytes` plus unknown/malformed transcript bytes, not raw internal-runtime byte growth. Never advance watermarks after a partial failed promotion.
