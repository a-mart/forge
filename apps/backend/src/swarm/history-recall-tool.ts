...
export function buildHistoryRecallTools(host: HistoryRecallToolHost, descriptor: AgentDescriptor): ToolDefinition[] {
  ...
  const sessionsOp = Type.Object({
    ...
    query: Type.Optional(Type.String({ minLength: 1, maxLength: 2000, description: 'Terms, quoted exact phrases, or code identifiers to find.' })),
  ...
  return [{
    ...
    parameters: host.listHistorySessions
      ? Type.Union([sessionsOp, searchOp, readOp])
      : Type.Union([searchOp, readOp]),
    async execute(_toolCallId, params) {
      const input = params as
        | ({ op: 'sessions' } & HistorySessionsRequest)
        | ({ op: 'search' } & HistorySearchRequest)
        | ({ op: 'read' } & HistoryReadRequest)
      let result: HistorySessionsResponse | HistorySearchResponse | HistoryReadResponse
