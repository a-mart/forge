...
recognized OpenSSH PEM keys have CRLF/CR newlines normalized to LF first so
Linux `ssh-add` can load keys pasted from Windows. The guest then loads the
keys into one foreground `ssh-agent` through `ssh-add -`,
