...
/** Leading plain @Codex / [@Codex] sidecar turn (not plugin selector). */
const LEADING_CODEX_SIDECAR_PATTERN =
  /^(?:@codex|\[@codex\])(?![-:\]])(?=\s|$)(?:\s([\s\S]*))?$/i;

/** Leading [@Codex:<selector>] internal plugin delegation hint. */
...
export type CodexUserMessageRoute =
  ...
  | { kind: "plugin_delegate"; selectors: string[]; strippedText: string };
...
export function classifyCodexUserMessage(text: string): CodexUserMessageRoute {
  ...
  if (leadingBracketTool) {
    ...
    return { kind: "plugin_delegate", selectors, strippedText: remainder };
  }
