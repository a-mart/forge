...
const EXCLUDE_FILE_CONTENT = [
  "# Auto-generated by Middleman versioning. Do not edit.",
...
export class EmbeddedGitVersioningService implements VersioningMutationSink {
  ...
  constructor(options: EmbeddedGitVersioningServiceOptions) {
    ...
    this.enabled = options.enabled ?? parseEnabledEnv(process.env.MIDDLEMAN_VERSIONING_ENABLED);
  ...
  private async ensureRepositoryInitialized(): Promise<void> {
    ...
    await this.git.run(["config", "user.name", "Middleman"]);
    await this.git.run(["config", "user.email", "middleman@local"]);
...
function parseTrackSessionMemoryEnv(): boolean {
  const normalized = process.env.MIDDLEMAN_VERSIONING_TRACK_SESSION_MEMORY?.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
...
function parseReconcileIntervalEnv(): number {
  const raw = process.env.MIDDLEMAN_VERSIONING_RECONCILE_INTERVAL_MS?.trim();
  if (!raw) {
